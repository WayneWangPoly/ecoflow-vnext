...
import {runAuthorizedImageCopyWindow22, IMAGE_COPY_WINDOW_22} from '../src/features/team/unleashedImageCopyWindow22.ts';

const predecessor={
  id:'2bf7da1f-741b-45af-8596-35b3660cfbb5',status:'SUCCEEDED',assets_planned:10,
  assets_copied:10,assets_reused:0,assets_failed:0,bytes_copied:959769,
  authorization_id:'9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c'
};
const result={runId:'w22',status:'SUCCEEDED',assetsPlanned:10,assetsCopied:10,assetsReused:0,assetsFailed:0,bytesCopied:100,errorCode:null,replayed:false};
...
test('exact W21 predecessor permits bounded W22 command',async()=>{
  const c=client();
  await runAuthorizedImageCopyWindow22(c);
  assert.equal(c.calls.length,1);
  assert.equal(c.calls[0][0],'trigger-unleashed-master-migration');
  assert.equal(c.calls[0][1].body.commandId,IMAGE_COPY_WINDOW_22.commandId);
  assert.equal(c.calls[0][1].body.limit,10);
});

test('mismatched W21 predecessor never invokes copy',async()=>{
  for(const run of [null,{...predecessor,status:'PARTIAL'},{...predecessor,assets_copied:9},{...predecessor,assets_failed:1},{...predecessor,bytes_copied:0},{...predecessor,authorization_id:'other'}]){
    const c=client(run);
    await assert.rejects(()=>runAuthorizedImageCopyWindow22(c));
...
test('malformed or over-limit response cannot report completed window',async()=>{
  for(const r of [{...result,status:'RUNNING'},{...result,assetsPlanned:11},{...result,assetsCopied:9},{...result,bytesCopied:-1}]){
    await assert.rejects(()=>runAuthorizedImageCopyWindow22(client(predecessor,r)));
  }
});

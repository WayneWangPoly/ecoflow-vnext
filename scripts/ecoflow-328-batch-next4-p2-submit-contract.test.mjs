...
import {
  BATCH_NEXT4_P2_TARGET,
  assertBatchNext4P2SubmitAcknowledgement,
  buildBatchNext4P2SubmitInput,
} from '../src/features/productIdentity/batchNext4P2ResumeSubmitContract.ts';

const carrier = readFileSync('src/features/productIdentity/BatchNext4P2ResumeSubmitCarrier.tsx', 'utf8');
const contract = readFileSync('src/features/productIdentity/batchNext4P2ResumeSubmitContract.ts', 'utf8');
const evidence = readFileSync('src/data/repositories/batchNext4P2ResumeEvidence.ts', 'utf8');
...
test('Batch Next 4 P2 target is frozen to exact production DRAFT rev10 and one unused SUBMIT command', () => {
  assert.equal(BATCH_NEXT4_P2_TARGET.protectedMainSha, 'faddc2d39bd053620f6d84c28d1cb3e09e0563f8');
  assert.equal(BATCH_NEXT4_P2_TARGET.batchId, 'f72ecd68-9177-4df7-ad18-c29a26e612f3');
  assert.equal(BATCH_NEXT4_P2_TARGET.batchName, 'ECOFLOW-328 Batch Next 4 DRAFT-only');
  assert.equal(BATCH_NEXT4_P2_TARGET.expectedRevision, 10);
  assert.equal(BATCH_NEXT4_P2_TARGET.startCommandId, 'f2bf897d-a1e6-54e2-909a-c58204960210');
  assert.equal(BATCH_NEXT4_P2_TARGET.submitCommandId, '3fa78212-ba4c-5318-8b35-55fcbbf103af');
  assert.equal(Object.keys(BATCH_NEXT4_P2_TARGET.identities).length, 10);
});

test('SUBMIT input and acknowledgement are exact and revisioned', () => {
  assert.deepEqual(buildBatchNext4P2SubmitInput(), {
  ...
  assert.doesNotThrow(() => assertBatchNext4P2SubmitAcknowledgement({
    batchId: BATCH_NEXT4_P2_TARGET.batchId,
    batchStatus: 'SUBMITTED',
    revision: 11,
    commandStatus: 'APPLIED',
  }));
  assert.throws(() => assertBatchNext4P2SubmitAcknowledgement({
    batchId: BATCH_NEXT4_P2_TARGET.batchId,
...
test('carrier performs authenticated SELECT evidence gate before exactly one incumbent SUBMIT', () => {
  ...
  const readEvidence = carrier.indexOf('await readBatchNext4P2ResumeEvidence()');
  const assertion = carrier.indexOf('assertBatchNext4P2ResumeEvidence(currentBatch, serverEvidence)');
...
test('NEXT4 P2 replaces completed DRAFT carrier on the native surface and remains CI-gated', () => {
  ...
  assert.match(workflow, /BatchNext4P2ResumeSubmitCarrier\.tsx/);
  assert.match(workflow, /batchNext4P2ResumeSubmitContract\.ts/);
  assert.match(workflow, /batchNext4P2ResumeEvidence\.ts/);
});

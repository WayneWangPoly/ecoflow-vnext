...
dashboard = replaceOnce(
  ...
  'dashboard contract import',
);
dashboard = replaceOnce(
  dashboard,
  '  onOpenOrders: () => void;\n',
  '  onOpenTab: (tab: DashboardNavigationTab) => void;\n',
  'dashboard callback prop',
);
dashboard = replaceOnce(
  dashboard,
  ...
  '',
  'legacy text navigation helper',
);
dashboard = replaceOnce(
  dashboard,
  'export function DashboardPage({ role, data, orders, snapshotReady, loading, loadError, healthNotice, onReload, onOpenOrders }: Props) {',
  'export function DashboardPage({ role, data, orders, snapshotReady, loading, loadError, healthNotice, onReload, onOpenTab }: Props) {',
  'dashboard signature',
);
dashboard = replaceOnce(
  dashboard,
...
dashboard = replaceOnce(
  dashboard,
  'onClick={onOpenOrders}>Review orders</button>',
  "onClick={() => onOpenTab('orders')}>Review orders</button>",
  'review orders navigation',
);
dashboard = replaceOnce(
  dashboard,
  "onClick={() => openSection('Reconciliation')}>Reconciliation</button>",
  "onClick={() => onOpenTab('reconciliation')}>Reconciliation</button>",
  'reconciliation navigation',
);
dashboard = replaceOnce(
  dashboard,
  'onClick={() => openSection(stage.section)}>',
  'onClick={() => onOpenTab(stage.tab)}>',
  'stage navigation',
);
dashboard = replaceOnce(
  dashboard,
  'onClick={onOpenOrders}>View all</button>',
  "onClick={() => onOpenTab('orders')}>View all</button>",
  'view all navigation',
);
...
app = replaceOnce(
  app,
  "onReload={onReload} onOpenOrders={() => setTab('orders')} />",
  'onReload={onReload} onOpenTab={setTab} />',
  'desktop shell dashboard navigation callback',
);

const contract = `import type { Role } from '@/domain/types';
...
if (!app.includes('onReload={onReload} onOpenTab={setTab} />')) throw new Error('INTEL_FE_001C_SHELL_CALLBACK_NOT_CONNECTED');
if (app.includes("onOpenOrders={() => setTab('orders')}")) throw new Error('INTEL_FE_001C_OLD_DASHBOARD_CALLBACK_REMAINS');
...
for (const testName of ['operational and ready stages open Orders', 'Accounts finance review opens Reconciliation', 'warehouse execution stages open Delivery']) {
...
if (fs.existsSync(path.join(root, 'scripts/materialise-intel-fe-001c.mjs'))) throw new Error('INTEL_FE_001C_MATERIALISER_REMAINS');
if (fs.existsSync(path.join(root, '.github/workflows/apply-intel-fe-001c-dashboard-navigation.yml'))) throw new Error('INTEL_FE_001C_TEMPORARY_WORKFLOW_REMAINS');
console.log('INTEL-FE-001C dashboard navigation audit passed.');
`;

const packageJson = JSON.parse(fs.readFileSync('package.json', 'utf8'));
packageJson.scripts['audit:intel-frontend'] += ' && node scripts/audit-intel-dashboard-navigation.mjs && node --experimental-strip-types --test scripts/intel-dashboard-navigation-contract.test.mjs';
packageJson.scripts['audit:operational-safety'] = 'node scripts/audit-operational-safety-continuity.mjs && npm run audit:intel-frontend';
const finalPackage = `${JSON.stringify(packageJson, null, 2)}\n`;

const bundle = {
  'src/app/App.tsx': app,
  'src/features/dashboard/DashboardPage.tsx': dashboard,
  'src/features/dashboard/dashboardNavigationContract.ts': contract,
  'scripts/intel-dashboard-navigation-contract.test.mjs': test,
  'scripts/audit-intel-dashboard-navigation.mjs': audit,
  'package.json': finalPackage,
};
const encoded = Buffer.from(JSON.stringify(bundle), 'utf8').toString('base64');
console.log('INTEL_FE_001C_BUNDLE_BEGIN');
console.log(encoded);
console.log('INTEL_FE_001C_BUNDLE_END');
process.exitCode = 1;

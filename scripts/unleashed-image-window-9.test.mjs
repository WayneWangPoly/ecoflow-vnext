...
import {runAuthorizedImageCopyWindow9, IMAGE_COPY_WINDOW_9} from '../src/features/team/unleashedImageCopyWindow9.ts';

const baseline={
  id:'e4dc80fd-9082-46b0-9d31-51a441d867a3',
  status:'SUCCEEDED',
  assets_planned:10,
  assets_copied:10,
  assets_reused:0,
  assets_failed:0,
  bytes_copied:1952538,
  authorization_id:'9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c'
};
const result={runId:'w9',status:'SUCCEEDED',assetsPlanned:10,assetsCopied:10,assetsReused:0,assetsFailed:0,bytesCopied:100,errorCode:null,replayed:false};
...
test('only exact successful predecessor permits bounded W9 command',async()=>{
  const c=client();
  await runAuthorizedImageCopyWindow9(c);
  assert.equal(c.calls.length,1);
  assert.equal(c.calls[0][0],'trigger-unleashed-master-migration');
  assert.equal(c.calls[0][1].body.commandId,IMAGE_COPY_WINDOW_9.commandId);
...
test('missing, failed, or mismatched predecessor never invokes copy',async()=>{
  for(const row of [
    ...
    const c=client(row);
    await assert.rejects(()=>runAuthorizedImageCopyWindow9(c));
...
test('malformed or running response cannot report completed window',async()=>{
  for(const r of [
    ...
    await assert.rejects(()=>runAuthorizedImageCopyWindow9(client(baseline,r)));
  }
});

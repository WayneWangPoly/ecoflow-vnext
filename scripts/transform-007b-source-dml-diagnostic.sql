...
create temp table transform_007b_source_dml_open on commit drop as
select
...
select
  ...
  relation_acl,
  rls_policies
from transform_007b_source_dml_open
...
begin
  select count(*) into v_count from transform_007b_source_dml_open;
  if v_count<>0 then

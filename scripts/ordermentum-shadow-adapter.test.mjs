...
import { buildListUrl, comparePair, replayTwice, selectDetailTarget, boundedJsonFetch, executeWindow } from './ordermentum-shadow-adapter.mjs';
...
test('request plan is exact and price groups omit invented supplier filter', () => {
...
test('pair comparison preserves order and replay is idempotent', () => {
...
test('pagination cap and provider interruption stop without retry or widening', async () => {
  ...
  await assert.rejects(() => executeWindow({ manifest, windowId: 'W0', supplierId: 'supplier-a', currentApiKey: 'current-key', legacyBearer: 'legacy-token', fetchImpl: async () => { calls += 1; return response({ data: ten }); } }), /partial after page cap/);
  assert.equal(calls, 4);
  calls = 0;
  await assert.rejects(() => executeWindow({ manifest, windowId: 'W0', supplierId: 'supplier-a', currentApiKey: 'current-key', legacyBearer: 'legacy-token', fetchImpl: async () => { calls += 1; throw new Error('offline interruption'); } }), /offline interruption/);

...
import {
  BATCH_NEXT4_P3_TARGET,
  assertBatchNext4P3PublishAcknowledgement,
  buildBatchNext4P3PublishInput,
  formatBatchNext4P3Failure,
} from '../src/features/productIdentity/batchNext4P3ResumePublishContract.ts';

const carrier = readFileSync('src/features/productIdentity/BatchNext4P3ResumePublishCarrier.tsx', 'utf8');
const contract = readFileSync('src/features/productIdentity/batchNext4P3ResumePublishContract.ts', 'utf8');
const evidence = readFileSync('src/data/repositories/batchNext4P3ResumeEvidence.ts', 'utf8');
...
test('Batch Next 4 P3 target is frozen to exact SUBMITTED rev11 and one unused PUBLISH command', () => {
  assert.equal(BATCH_NEXT4_P3_TARGET.protectedMainSha, '016f8d2b7a412979e67d449da0c19dd7975e1e65');
  assert.equal(BATCH_NEXT4_P3_TARGET.batchId, 'f72ecd68-9177-4df7-ad18-c29a26e612f3');
  assert.equal(BATCH_NEXT4_P3_TARGET.expectedRevision, 11);
  assert.equal(BATCH_NEXT4_P3_TARGET.submitCommandId, '3fa78212-ba4c-5318-8b35-55fcbbf103af');
  assert.equal(BATCH_NEXT4_P3_TARGET.publishCommandId, 'be8663c9-cfff-41f1-abb6-fc71a8ca2ffa');
  assert.deepEqual(Object.keys(BATCH_NEXT4_P3_TARGET.identities), [
    'BSB42LPLA',
    'CCEA12-90',
    'CCSW12-80',
    'IC4BOX',
...
test('PUBLISH input and acknowledgement are exact', () => {
  assert.deepEqual(buildBatchNext4P3PublishInput(), {
  ...
  assert.doesNotThrow(() => assertBatchNext4P3PublishAcknowledgement(accepted));
  assert.doesNotThrow(() => assertBatchNext4P3PublishAcknowledgement({ ...accepted, commandStatus: 'REPLAYED' }));
  for (const invalid of [
    ...
    { publishedLinks: 9 },
    { publishedAt: null },
  ]) assert.throws(() => assertBatchNext4P3PublishAcknowledgement({ ...accepted, ...invalid }), /acknowledgement/i);
});
...
test('carrier performs exactly one incumbent PUBLISH between exact preflight and postflight', () => {
  ...
  const preRead = carrier.indexOf("await readBatchNext4P3ResumeEvidence('PRE')");
  const preAssert = carrier.indexOf('assertBatchNext4P3Preflight(currentBatch, serverEvidence)');
  ...
  const postRead = carrier.indexOf("await readBatchNext4P3ResumeEvidence('POST')");
  const postAssert = carrier.indexOf('assertBatchNext4P3Postflight(postflight)');
...
test('post-boundary failure wording prohibits blind retry', () => {
  assert.match(formatBatchNext4P3Failure(new Error('read failed'), false), /PUBLISH was not called\.$/);
  const uncertain = formatBatchNext4P3Failure(new Error('network uncertain'), true);
...
test('completed NEXT4 P3 is archived when NEXT5 DRAFT becomes active, while NEXT4 P3 remains CI-gated', () => {
  ...
  assert.match(workflow, /batchNext4P3ResumeEvidence\.ts/);

...
import {runAuthorizedImageCopyWindow7, IMAGE_COPY_WINDOW_7} from '../src/features/team/unleashedImageCopyWindow7.ts';

const baseline={
  id:'a2bc3cc4-602f-4434-a653-f23bb16133a0',
  status:'SUCCEEDED',
  assets_planned:10,
  assets_copied:10,
  assets_reused:0,
  assets_failed:0,
  bytes_copied:1735224,
  authorization_id:'9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c'
};
const result={runId:'w7',status:'SUCCEEDED',assetsPlanned:10,assetsCopied:10,assetsReused:0,assetsFailed:0,bytesCopied:100,errorCode:null,replayed:false};
...
test('only exact successful predecessor permits bounded W7 command',async()=>{
  const c=client();
  await runAuthorizedImageCopyWindow7(c);
  assert.equal(c.calls.length,1);
  assert.equal(c.calls[0][0],'trigger-unleashed-master-migration');
  assert.equal(c.calls[0][1].body.commandId,IMAGE_COPY_WINDOW_7.commandId);
...
test('missing, failed, or mismatched predecessor never invokes copy',async()=>{
  for(const row of [
    ...
    const c=client(row);
    await assert.rejects(()=>runAuthorizedImageCopyWindow7(c));
...
test('malformed or running response cannot report completed window',async()=>{
  for(const r of [
    ...
    await assert.rejects(()=>runAuthorizedImageCopyWindow7(client(baseline,r)));
  }
});

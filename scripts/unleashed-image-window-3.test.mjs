...
import {runAuthorizedImageCopyWindow3, IMAGE_COPY_WINDOW_3} from '../src/features/team/unleashedImageCopyWindow3.ts';

const baseline={
  id:'6309ef23-fc1c-4c3e-9b25-db32707dbd34',
  status:'SUCCEEDED',
  assets_planned:10,
  assets_copied:10,
  assets_reused:0,
  assets_failed:0,
  bytes_copied:7289531,
  authorization_id:'9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c'
};
const result={runId:'w3',status:'SUCCEEDED',assetsPlanned:10,assetsCopied:10,assetsReused:0,assetsFailed:0,bytesCopied:100,errorCode:null,replayed:false};
...
test('only exact successful predecessor permits bounded W3 command',async()=>{
  const c=client();
  await runAuthorizedImageCopyWindow3(c);
  assert.equal(c.calls.length,1);
  assert.equal(c.calls[0][0],'trigger-unleashed-master-migration');
  assert.equal(c.calls[0][1].body.commandId,IMAGE_COPY_WINDOW_3.commandId);
...
test('missing, failed, or mismatched predecessor never invokes copy',async()=>{
  for(const row of [
    ...
    const c=client(row);
    await assert.rejects(()=>runAuthorizedImageCopyWindow3(c));
...
test('malformed or running response cannot report completed window',async()=>{
  for(const r of [
    ...
    await assert.rejects(()=>runAuthorizedImageCopyWindow3(client(baseline,r)));
  }
});

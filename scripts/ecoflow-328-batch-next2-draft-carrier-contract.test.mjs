...
import {
  ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET,
  buildBatchNext2ReconcileInput,
  buildBatchNext2StartInput,
} from '../src/features/productIdentity/batchNext2DraftOnlyContract.ts';

const carrier = readFileSync('src/features/productIdentity/BatchNext2DraftOnlyCarrier.tsx', 'utf8');
const evidence = readFileSync('src/data/repositories/batchNext2DraftResumeEvidence.ts', 'utf8');
...
test('Batch Next 2 freezes ten distinct high-impact Survey candidates', () => {
  const target = ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET;
  assert.equal(target.protectedMainSha, '5a634ed729520534fd29803d654bb267c7274c74');
  assert.equal(target.batchName, 'ECOFLOW-328 Batch Next 2 DRAFT-only');
  assert.equal(target.startCommandId, 'f93b5ce7-80d0-4b41-a756-d2fd382827dd');
  assert.equal(target.referenceBatchId, '4cdb85d3-06d8-44bf-96bb-93660e10c3c9');
  assert.equal(target.candidates.length, 10);
  assert.equal(target.totalReferenceQty, 765);
  ...
  assert.equal(target.candidates.reduce((sum, x) => sum + x.referenceQty, 0), 765);
});

test('start input is exactly one bounded ten-SKU scope', () => {
  const input = buildBatchNext2StartInput();
  assert.equal(input.batchName, ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET.batchName);
  assert.equal(input.commandId, ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET.startCommandId);
  assert.deepEqual(input.commercialSkuIds, ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET.candidates.map((x) => x.commercialSkuId));
});

test('reconcile input is frozen CARTON x1 / PROHIBITED / preferred and requires explicit confirmation', () => {
  const candidate = ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET.candidates[0];
  assert.throws(
    () => buildBatchNext2ReconcileInput(candidate, '11111111-1111-4111-8111-111111111111', false),
    /explicit Owner\/Admin confirmation/,
  );
  const input = buildBatchNext2ReconcileInput(candidate, '11111111-1111-4111-8111-111111111111', true);
...
test('carrier is DRAFT-only and supports exact DRAFT resume after refresh', () => {
  assert.match(carrier, /readBatchNext2ResumeEvidence/);
...
test('native surface lazy-loads Batch Next 2 and CI gates it', () => {
  assert.match(wrapper, /lazy\(\(\) => import\('\.\/BatchNext2DraftOnlyCarrier'\)/);
  assert.match(wrapper, /<BatchNext2DraftOnlyCarrier/);
  assert.match(wrapper, /<Suspense fallback=\{null\}>/);
  assert.match(workflow, /ecoflow-328-batch-next2-draft-carrier-contract\.test\.mjs/);
  assert.match(workflow, /BatchNext2DraftOnlyCarrier\.tsx/);
  assert.match(workflow, /batchNext2DraftOnlyContract\.ts/);
  assert.match(workflow, /batchNext2DraftResumeEvidence\.ts/);
});

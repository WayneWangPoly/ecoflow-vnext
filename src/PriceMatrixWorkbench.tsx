...
function PriceMatrixContent() {
  ...
  const skuRows = useMemo(() => {
    ...
    return [...bySku.values()].slice(0, 180);
  }, [rows, query]);
  ...
  return <section className="price-matrix-shell">
    <header className="price-matrix-hero"><div><span>COMMERCIAL CONTROL</span><h2>SKU price matrix</h2><p>EcoFlow target prices by Ordermentum price group. Every change is versioned; this does not silently push prices back to Ordermentum.</p></div><div><strong>{groups.length}</strong><span>synced price groups</span><button type="button" onClick={() => void reload()} disabled={busy}>Refresh</button></div></header>
    {groups.length !== 4 ? <div className="price-matrix-warning">Owner expected four customer price tiers; {groups.length} are currently mirrored from Ordermentum. Sync price groups before treating this matrix as complete.</div> : null}

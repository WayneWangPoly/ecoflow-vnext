...
import {
  executeLegacyInternalReviewDecision,
...
function SchemaRow({ row }: { row: InternalOrderSchemaProbeRow }) {
  const important = String(row.columns || '').includes('internal_order_id') || String(row.columns || '').includes('internalisation_status') || row.table_name === 'ecoflow_ordermentum_internal_orders';
  return (
...
function DependencyRow({ row }: { row: InternalOrderDependencyRow }) {
  ...
  return (
    <article className="internal-exec-dependency-row">
      <div><strong>{row.object_name}</strong><span>{row.object_type} · {row.execution_role || 'REFERENCE'}</span></div>
      <ReadinessPill tone={ready ? 'good' : 'warn'}>{ready ? 'execution candidate' : 'needs review'}</ReadinessPill>
      <small>{row.has_internal_order_id ? `identity: ${row.identity_column || 'internal_order_id'}` : 'no internal identity'} · {row.has_internalisation_status ? `status: ${row.status_column || 'internalisation_status'}` : 'no status'} · {row.has_warehouse_gate_status ? 'warehouse_gate_status' : 'no warehouse_gate_status'}</small>
    </article>
  );
}

function QueueRow({ row, busy, onExecute }: { row: InternalOrderExecutionQueueRow; busy: boolean; onExecute: (row: InternalOrderExecutionQueueRow) => void }) {
  const done = Boolean(row.executed_at) || row.execution_status === 'ARCHIVED_NO_TABLE_MUTATION' || row.execution_status === 'CANCELLED_LEGACY_DRAFT' || row.execution_status === 'OLD_DRAFT_HELD_FOR_REBUILD';
  const failed = row.execution_status === 'EXECUTION_FAILED' || Boolean(row.error_message);
  const tone = failed ? 'warn' : done ? 'good' : row.execution_status?.includes('READY') || row.execution_status?.includes('PENDING') ? 'blue' : 'neutral';
  return (
    <article className="internal-exec-queue-row">
      ...
      <small>{title(row.decision)} · decided {timeText(row.decided_at)}{row.executed_at ? ` · executed ${timeText(row.executed_at)}` : ''}{row.error_message ? ` · ${row.error_message}` : ''}</small>
      <button className="internal-exec-run-button" type="button" disabled={busy || done || !row.lifecycle_id} onClick={() => onExecute(row)}>{busy ? 'Executing…' : done ? 'Executed' : 'Execute decision'}</button>
    </article>
  );
}
...
function InspectorContent() {
  ...
  async function executeRow(row: InternalOrderExecutionQueueRow) {
    if (!row.lifecycle_id) return;
    setBusyId(row.lifecycle_id);
    setError('');
    setNotice('');
    try {
      const result = await executeLegacyInternalReviewDecision(row.lifecycle_id);
      const first = result[0];
      setNotice(`${row.order_number || row.lifecycle_id}: ${title(first?.execution_status || 'EXECUTED')}.`);
      await reload();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
  ...
  return (
    <section className="internal-exec-shell">
      <section className="internal-exec-hero">
        <div><span>INTERNAL ORDER EXECUTION READINESS</span><h3>{readiness}</h3><p>The internal order source is confirmed when the base table exposes an internal identity, status, and warehouse gate. Executions are audited with before/after snapshots.</p></div>
      ...
      {notice ? <div className="internal-exec-notice">{notice}</div> : null}
      ...
      <section className="internal-exec-grid">
        ...
        <div className="internal-exec-panel"><header><h4>Execution queue</h4><ReadinessPill tone={queue.length ? 'blue' : 'neutral'}>{queue.length}</ReadinessPill></header><div className="internal-exec-list">{queue.slice(0, 8).map((row) => <QueueRow key={`${row.id}-${row.lifecycle_id}`} row={row} busy={busyId === row.lifecycle_id} onExecute={executeRow} />)}{!queue.length ? <div className="internal-exec-empty">No archive/cancel/rebuild decisions waiting for execution.</div> : null}</div></div>
      </section>

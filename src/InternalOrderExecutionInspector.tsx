...
function ReadinessPill({ children, tone = 'neutral' }: { children: React.ReactNode; tone?: 'good' | 'warn' | 'blue' | 'neutral' }) {
  ...
  return (
    <article className="internal-exec-schema-row">
      <div>
        ...
        <span>{row.table_type} · {n(row.column_count)} columns</span>
      </div>
...
function DependencyRow({ row }: { row: InternalOrderDependencyRow }) {
  ...
  return (
    <article className="internal-exec-dependency-row">
      ...
      <small>
        {row.has_internal_order_id ? 'internal_order_id' : 'no internal_order_id'} · {row.has_internalisation_status ? 'internalisation_status' : 'no internalisation_status'} · {row.has_warehouse_gate_status ? 'warehouse_gate_status' : 'no warehouse_gate_status'}
      </small>
    </article>
  );
}
...
function QueueRow({ row }: { row: InternalOrderExecutionQueueRow }) {
  ...
  return (
    <article className="internal-exec-queue-row">
      <div>
        <strong>{row.order_number || row.lifecycle_id}</strong>
        <span>{row.invoice_number || 'invoice pending'} · {row.internal_order_id || 'no internal id'}</span>
      </div>
  ...
  return (
    <section className="internal-exec-shell">
      <section className="internal-exec-hero">
        <div>
          <span>INTERNAL ORDER EXECUTION READINESS</span>
          <h3>{readiness}</h3>
          <p>I can determine the next destructive step from database metadata once the probe migration has run. Until the source table is confirmed, cancel/rebuild stays as an audited request, not a blind delete.</p>
        </div>
        <div className="internal-exec-actions">
          <button type="button" onClick={() => void reload()}>Refresh readiness</button>
          <small>{loadedAt ? `checked ${timeText(loadedAt)}` : 'waiting for probe'}</small>
        </div>
      </section>
      ...
      <section className="internal-exec-metrics">
        <div><strong>{schema.length}</strong><span>schema objects</span></div>
        <div><strong>{dependencies.length}</strong><span>draft dependencies</span></div>
        <div><strong>{executionCandidates.length}</strong><span>execution candidates</span></div>
        <div><strong>{queue.length}</strong><span>decision queue</span></div>
      </section>

      <section className="internal-exec-grid">
        <div className="internal-exec-panel">
          <header><h4>Draft view dependencies</h4><ReadinessPill tone={executionCandidates.length ? 'good' : 'warn'}>{executionCandidates.length ? 'candidate found' : 'not confirmed'}</ReadinessPill></header>
          <div className="internal-exec-list">
            {dependencies.slice(0, 8).map((row) => <DependencyRow key={`${row.object_type}-${row.object_name}`} row={row} />)}
            {!dependencies.length ? <div className="internal-exec-empty">Run the readiness migration to inspect dependencies.</div> : null}
          </div>
        </div>

        <div className="internal-exec-panel">
          <header><h4>Execution queue</h4><ReadinessPill tone={queue.length ? 'blue' : 'neutral'}>{queue.length}</ReadinessPill></header>
          <div className="internal-exec-list">
            {queue.slice(0, 8).map((row) => <QueueRow key={`${row.id}-${row.lifecycle_id}`} row={row} />)}
            {!queue.length ? <div className="internal-exec-empty">No archive/cancel/rebuild decisions waiting for execution.</div> : null}
          </div>
        </div>
      </section>

      <section className="internal-exec-panel">
        <header><h4>Schema probe</h4><ReadinessPill tone={schema.length ? 'blue' : 'neutral'}>{schema.length}</ReadinessPill></header>
        <div className="internal-exec-schema-grid">
          {schema.slice(0, 10).map((row) => <SchemaRow key={`${row.table_type}-${row.table_name}`} row={row} />)}
          {!schema.length ? <div className="internal-exec-empty">No schema rows loaded yet.</div> : null}
        </div>
      </section>
    </section>
  );
...
export function InternalOrderExecutionInspector() {

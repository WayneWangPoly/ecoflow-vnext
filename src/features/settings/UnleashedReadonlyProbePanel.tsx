...
import {
  runAuthorizedImageCopyWindow37,
  type AuthorizedImageCopyWindowResult,
} from '../team/unleashedImageCopyWindow37';
...
export function UnleashedReadonlyProbePanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  async function runCopyWindow37() {
    ...
    try { setCopyResult(await runAuthorizedImageCopyWindow37(supabase)); }
  ...
  return (
    <section className="panel unleashed-probe-panel">
      <div className="panel-head"><div><h2>Unleashed connection</h2><span>GET-only source · #338 governed migration controls</span></div><b className={`pill pill-${probeTone(result)}`}>{running ? 'RUNNING' : result?.status ?? 'NOT TESTED'}</b></div>
      ...
      <div className="system-status-grid">
        ...
        <div><span>Currently exposed</span><strong>COPY_IMAGES window 37 only · max 10 assets</strong></div>
      </div>
      <div className="system-sync-actions unleashed-probe-actions">
        ...
        <button type="button" className="primary" onClick={() => void runCopyWindow37()} disabled={anyRunning || copyAttempted || Boolean(copyResult)}><Database aria-hidden="true" size={17} />{copyRunning ? 'Copying bounded image window 37…' : copyResult ? 'Image window 37 completed' : copyAttempted ? 'Image window 37 attempt sent' : 'Execute authorized #338 COPY_IMAGES window 37'}</button>
      </div>
      <p className="unleashed-acceptance-note">W36 was production-verified as 10 copied / 0 failed / 3376311 bytes with clean Storage/provenance integrity. Production currently has 350 private objects / 88815566 bytes, 85 PLANNED, 32 terminal BLOCKED and zero active claims. Authorization revision 2 remains current at 128 MiB aggregate and 2 MiB per object with unchanged rights scope. This thirty-seventh window is capped at 10 eligible assets and uses one fixed command id. No continuation window is exposed until production verification.</p>
      {copyError ? <div className="error-message" role="alert">{copyError}</div> : null}
      {copyResult ? <div className="unleashed-acceptance-result" role="status"><div className="unleashed-acceptance-checks"><div><span><strong>#338 COPY_IMAGES window 37</strong><small>{copyResult.assetsPlanned} planned · {copyResult.assetsCopied} copied · {copyResult.assetsReused} reused · {copyResult.assetsFailed} failed · {copyResult.bytesCopied} bytes</small></span><b className={`pill ${copyResult.status === 'SUCCEEDED' ? 'pill-good' : 'pill-warning'}`}>{copyResult.status}</b></div></div></div> : null}

...
import {
  runAuthorizedImageCopyWindow2,
  type AuthorizedImageCopyWindowResult,
} from '../team/unleashedImageCopyWindow2';
...
export function UnleashedReadonlyProbePanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  async function runCopyWindow2() {
    ...
    setCopyError('');
    try {
      setCopyResult(await runAuthorizedImageCopyWindow2(supabase));
    } catch (runError) {
  ...
  return (
    <section className="panel unleashed-probe-panel">
      ...
      <div className="system-status-grid">
        ...
        <div><span>Currently exposed</span><strong>COPY_IMAGES window 2 only · max 10 assets</strong></div>
      </div>
      ...
      <div className="system-sync-actions unleashed-probe-actions">
        ...
        <button type="button" className="primary" onClick={() => void runCopyWindow2()} disabled={anyRunning || copyAttempted || Boolean(copyResult)}>
          <Database aria-hidden="true" size={17} />
          {copyRunning ? 'Copying bounded image window 2…' : copyResult ? 'Image window 2 completed' : copyAttempted ? 'Image window 2 attempt sent' : 'Execute authorized #338 COPY_IMAGES window 2'}
        </button>
      </div>

      <p className="unleashed-acceptance-note">
        The production image authorization is current and APPROVED at revision 1. This action is the second binary-copy window, following verified W1 (10 copied, 960132 bytes, no failures) and is capped at 10 planned assets. It uses one fixed command id so a browser retry cannot create a second run. The Edge Function re-checks rights, source snapshot hash, HTTPS host, MIME/content signature, the 2 MiB per-object limit and the 64 MiB aggregate budget before committing provenance. No continuation window is exposed until production verification of copied/reused/failed counts and actual bytes. Product Identity, inventory/opening balance and cutover remain dependency-gated.
      </p>
      ...
      {copyResult ? (
        <div className="unleashed-acceptance-result" role="status">
          <div className="unleashed-acceptance-checks">
            <div>
              <span>
                <strong>#338 COPY_IMAGES window 2</strong>

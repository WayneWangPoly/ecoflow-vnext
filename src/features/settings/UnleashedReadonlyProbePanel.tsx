...
import {
  runAuthorizedImageCopyWindow20,
  type AuthorizedImageCopyWindowResult,
} from '../team/unleashedImageCopyWindow20';
...
export function UnleashedReadonlyProbePanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  async function runCopyWindow20() {
    ...
    setCopyError('');
    try {
      setCopyResult(await runAuthorizedImageCopyWindow20(supabase));
    } catch (runError) {
  ...
  return (
    <section className="panel unleashed-probe-panel">
      ...
      <div className="system-status-grid">
        ...
        <div><span>Currently exposed</span><strong>COPY_IMAGES window 20 only · max 10 assets</strong></div>
      </div>
      ...
      <div className="system-sync-actions unleashed-probe-actions">
        ...
        <button type="button" className="primary" onClick={() => void runCopyWindow20()} disabled={anyRunning || copyAttempted || Boolean(copyResult)}>
          <Database aria-hidden="true" size={17} />
          {copyRunning ? 'Copying bounded image window 20…' : copyResult ? 'Image window 20 completed' : copyAttempted ? 'Image window 20 attempt sent' : 'Execute authorized #338 COPY_IMAGES window 20'}
        </button>
      </div>

      <p className="unleashed-acceptance-note">
        The production image authorization is current and APPROVED at revision 1. W19 was production-verified as 10 copied / 0 failed / 2203130 bytes. Production Storage currently contains 189 private objects / 51465323 bytes, 28 assets are terminal BLOCKED, 250 remain PLANNED, and no asset claims remain. This twentieth window is capped at 10 planned assets and uses one fixed command id so a browser retry cannot create a second logical run. The Edge Function re-checks rights, source snapshot hash, HTTPS host, MIME/content signature, the 2 MiB per-object limit and the 64 MiB aggregate budget before committing provenance. No continuation window is exposed until production verification. Product Identity, inventory/opening balance and cutover remain dependency-gated.
      </p>
      ...
      {copyResult ? (
        <div className="unleashed-acceptance-result" role="status">
          <div className="unleashed-acceptance-checks">
            <div>
              <span>
                <strong>#338 COPY_IMAGES window 20</strong>

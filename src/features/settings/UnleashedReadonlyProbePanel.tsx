...
import {
  runAuthorizedImageCopyWindow10,
  type AuthorizedImageCopyWindowResult,
} from '../team/unleashedImageCopyWindow10';
...
export function UnleashedReadonlyProbePanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  async function runCopyWindow10() {
    ...
    setCopyError('');
    try {
      setCopyResult(await runAuthorizedImageCopyWindow10(supabase));
    } catch (runError) {
  ...
  return (
    <section className="panel unleashed-probe-panel">
      ...
      <div className="system-status-grid">
        ...
        <div><span>Currently exposed</span><strong>COPY_IMAGES window 10 only · max 10 assets</strong></div>
      </div>
      ...
      {copyResult ? (
        <div className="unleashed-acceptance-result" role="status">
          <div className="unleashed-acceptance-checks">
            <div>
              <span>
                <strong>#338 COPY_IMAGES window 10</strong>

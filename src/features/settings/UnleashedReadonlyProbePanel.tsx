...
import {
  runAuthorizedImageCopyWindow16,
  type AuthorizedImageCopyWindowResult,
} from '../team/unleashedImageCopyWindow16';
...
export function UnleashedReadonlyProbePanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  async function runCopyWindow16() {
    ...
    setCopyError('');
    try {
      setCopyResult(await runAuthorizedImageCopyWindow16(supabase));
    } catch (runError) {
  ...
  return (
    <section className="panel unleashed-probe-panel">
      ...
      <div className="system-status-grid">
        ...
        <div><span>Currently exposed</span><strong>COPY_IMAGES window 16 only · max 10 assets</strong></div>
      </div>
      ...
      {copyResult ? (
        <div className="unleashed-acceptance-result" role="status">
          <div className="unleashed-acceptance-checks">
            <div>
              <span>
                <strong>#338 COPY_IMAGES window 16</strong>

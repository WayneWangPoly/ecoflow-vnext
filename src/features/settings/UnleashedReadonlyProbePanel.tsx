...
import {
  runAuthorizedImageCopyWindow35,
  type AuthorizedImageCopyWindowResult,
} from '../team/unleashedImageCopyWindow35';
...
export function UnleashedReadonlyProbePanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  async function runCopyWindow35() {
    ...
    try { setCopyResult(await runAuthorizedImageCopyWindow35(supabase)); }
  ...
  return (
    <section className="panel unleashed-probe-panel">
      ...
      <div className="system-status-grid">
        ...
        <div><span>Currently exposed</span><strong>COPY_IMAGES window 35 only · max 10 assets</strong></div>
      </div>
      <div className="system-sync-actions unleashed-probe-actions">
        ...
        <button type="button" className="primary" onClick={() => void runCopyWindow35()} disabled={anyRunning || copyAttempted || Boolean(copyResult)}><Database aria-hidden="true" size={17} />{copyRunning ? 'Copying bounded image window 35…' : copyResult ? 'Image window 35 completed' : copyAttempted ? 'Image window 35 attempt sent' : 'Execute authorized #338 COPY_IMAGES window 35'}</button>
      </div>
      <p className="unleashed-acceptance-note">W34 was production-verified as 10 copied / 0 failed / 734142 bytes with clean Storage/provenance integrity. Production currently has 330 private objects / 83700853 bytes, 105 PLANNED, 32 terminal BLOCKED and zero active claims. Authorization revision 2 remains current at 128 MiB aggregate and 2 MiB per object with unchanged rights scope. This thirty-fifth window is capped at 10 eligible assets and uses one fixed command id. No continuation window is exposed until production verification.</p>
      {copyError ? <div className="error-message" role="alert">{copyError}</div> : null}
      {copyResult ? <div className="unleashed-acceptance-result" role="status"><div className="unleashed-acceptance-checks"><div><span><strong>#338 COPY_IMAGES window 35</strong><small>{copyResult.assetsPlanned} planned · {copyResult.assetsCopied} copied · {copyResult.assetsReused} reused · {copyResult.assetsFailed} failed · {copyResult.bytesCopied} bytes</small></span><b className={`pill ${copyResult.status === 'SUCCEEDED' ? 'pill-good' : 'pill-warning'}`}>{copyResult.status}</b></div></div></div> : null}

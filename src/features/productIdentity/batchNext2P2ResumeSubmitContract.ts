export type BatchNext2P2CurrentBatch = {
  batchId: string;
  batchName?: string;
  batchStatus: string;
  revision: number;
...
export type BatchNext2P2EvidenceRow = Record<string, unknown>;

export type BatchNext2P2Evidence = {
  batches: BatchNext2P2EvidenceRow[];
  scopeItems: BatchNext2P2EvidenceRow[];
  reconciliations: BatchNext2P2EvidenceRow[];
  observations: BatchNext2P2EvidenceRow[];
  families: BatchNext2P2EvidenceRow[];
  physicalSkus: BatchNext2P2EvidenceRow[];
  packages: BatchNext2P2EvidenceRow[];
  barcodeBindings: BatchNext2P2EvidenceRow[];
  commercialFamilyLinks: BatchNext2P2EvidenceRow[];
};

export type BatchNext2P2SubmitAcknowledgement = {
  batchId: string;
  batchStatus: string;
  revision: number;
...
export const BATCH_NEXT2_P2_TARGET = {
  protectedMainSha: 'be53d50c9978d097443eafa658cb88cacd3a4ed1',
  ...
  submitNote: 'ECOFLOW-328-BATCH-NEXT2-P2 SUBMIT only; exact ten-SKU DRAFT graph independently verified; PUBLISH, barcode reassignment/retirement, inventory/stocktake/location mutation and provider traffic not authorized.',
  identities: {
    'CCEA16-90': {
...
type Identity = (typeof BATCH_NEXT2_P2_TARGET.identities)[keyof typeof BATCH_NEXT2_P2_TARGET.identities];

function hold(detail: string): never {
  throw new Error(`HOLD — ${detail}; SUBMIT was not called.`);
}

function requireCount(rows: BatchNext2P2EvidenceRow[], expected: number, label: string) {
  if (rows.length !== expected) hold(`${label} must contain exactly ${expected} rows`);
}

function expectField(row: BatchNext2P2EvidenceRow, field: string, expected: unknown, label: string) {
...
function findExactly(rows: BatchNext2P2EvidenceRow[], field: string, value: string, label: string) {
  const matches = rows.filter((row) => row[field] === value);
  if (matches.length !== 1) hold(`${label} must contain exactly one row for ${value}`);
  return matches[0];
}

function assertCurrentBatch(currentBatch: BatchNext2P2CurrentBatch | null) {
  if (!currentBatch) hold('current batch authority returned no batch');
  if (currentBatch.batchId !== BATCH_NEXT2_P2_TARGET.batchId) hold('current batch ID mismatch');
  if (currentBatch.batchName !== BATCH_NEXT2_P2_TARGET.batchName) hold('current batch name mismatch');
  if (currentBatch.batchStatus !== 'DRAFT') hold('current batch must be DRAFT');
  if (currentBatch.revision !== BATCH_NEXT2_P2_TARGET.expectedRevision) hold('current batch must be revision 10');
...
function assertBatchRow(rows: BatchNext2P2EvidenceRow[]) {
  requireCount(rows, 1, 'batch evidence');
  const row = rows[0];
  for (const [field, expected] of Object.entries({
    id: BATCH_NEXT2_P2_TARGET.batchId,
    batch_name: BATCH_NEXT2_P2_TARGET.batchName,
    batch_status: 'DRAFT',
    revision: BATCH_NEXT2_P2_TARGET.expectedRevision,
    start_command_id: BATCH_NEXT2_P2_TARGET.startCommandId,
...
function assertScope(rows: BatchNext2P2EvidenceRow[]) {
  ...
  for (const identity of Object.values(BATCH_NEXT2_P2_TARGET.identities)) {
    ...
    expectField(row, 'batch_id', BATCH_NEXT2_P2_TARGET.batchId, 'scope evidence');
    expectField(row, 'start_command_id', BATCH_NEXT2_P2_TARGET.startCommandId, 'scope evidence');
  }
}

function assertIdentity(evidence: BatchNext2P2Evidence, code: string, identity: Identity) {
  const reconciliation = findExactly(evidence.reconciliations, 'id', identity.reconciliationId, `${code} reconciliation`);
  for (const [field, expected] of Object.entries({
    batch_id: BATCH_NEXT2_P2_TARGET.batchId,
  ...
  for (const [field, expected] of Object.entries({
    ...
    created_in_batch_id: BATCH_NEXT2_P2_TARGET.batchId,
...
export function assertBatchNext2P2ResumeEvidence(
  currentBatch: BatchNext2P2CurrentBatch | null,
  evidence: BatchNext2P2Evidence,
) {
  ...
  for (const [code, identity] of Object.entries(BATCH_NEXT2_P2_TARGET.identities)) {
    assertIdentity(evidence, code, identity);
  }
}

export function buildBatchNext2P2SubmitInput() {
  return {
    batchId: BATCH_NEXT2_P2_TARGET.batchId,
    expectedRevision: BATCH_NEXT2_P2_TARGET.expectedRevision,
    commandId: BATCH_NEXT2_P2_TARGET.submitCommandId,
    note: BATCH_NEXT2_P2_TARGET.submitNote,
  };
}

export function assertBatchNext2P2SubmitAcknowledgement(result: BatchNext2P2SubmitAcknowledgement) {
  if (
    result.batchId !== BATCH_NEXT2_P2_TARGET.batchId
    || result.batchStatus !== 'SUBMITTED'
    || result.revision !== 11
    || !['APPLIED', 'REPLAYED'].includes(result.commandStatus)
  ) throw new Error('Batch Next 2 P2 SUBMIT acknowledgement is not the exact SUBMITTED rev11 APPLIED/REPLAYED result.');
}

export function formatBatchNext2P2Failure(error: unknown, commandCrossedBoundary: boolean) {

...
import { readBatchNext4P3ResumeEvidence } from '@/data/repositories/batchNext4P3ResumeEvidence';
import {
...
import {
  ...
  assertBatchNext4P3Postflight,
  assertBatchNext4P3Preflight,
  assertBatchNext4P3PublishAcknowledgement,
  buildBatchNext4P3PublishInput,
  formatBatchNext4P3Failure,
  type BatchNext4P3Evidence,
} from './batchNext4P3ResumePublishContract';

type Props = { role: Role; onChanged: () => void };

export function BatchNext4P3ResumePublishCarrier({ role, onChanged }: Props) {
  ...
  const [preflightEvidence, setPreflightEvidence] = useState<BatchNext4P3Evidence | null>(null);
  const [postflightEvidence, setPostflightEvidence] = useState<BatchNext4P3Evidence | null>(null);
  const [result, setResult] = useState<ProductIdentityPublishResult | null>(null);
  const [message, setMessage] = useState('No Batch Next 4 P3 PUBLISH command has been sent.');
  ...
  async function resumeAndPublish() {
    ...
    try {
      ...
      const serverEvidence = await readBatchNext4P3ResumeEvidence('PRE');
      assertBatchNext4P3Preflight(currentBatch, serverEvidence);
      setPreflight(currentBatch);
      setPreflightEvidence(serverEvidence);

      const input = buildBatchNext4P3PublishInput();
      commandCrossedBoundary = true;
      setCommandAttempted(true);
      const acknowledgement = await publishProductIdentityBatch(input);
      assertBatchNext4P3PublishAcknowledgement(acknowledgement);

      const postflight = await readBatchNext4P3ResumeEvidence('POST');
      assertBatchNext4P3Postflight(postflight);
      ...
      setMessage(formatBatchNext4P3Failure(error, commandCrossedBoundary));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next 4 P3 publish-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · TEN-SKU PUBLISH-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next 4 P3 PUBLISH gate</h2>
      ...
      <div className="survey-reconciliation-layout">
        <fieldset className="survey-reconciliation-form" disabled={busy || commandAttempted || result !== null}>
          ...
          <dl className="survey-reconciliation-evidence survey-reconciliation-note">
            <div><dt>Protected main</dt><dd>{BATCH_NEXT4_P3_TARGET.protectedMainSha}</dd></div>
            <div><dt>Batch ID</dt><dd>{BATCH_NEXT4_P3_TARGET.batchId}</dd></div>
            <div><dt>Required state</dt><dd>SUBMITTED revision 11</dd></div>
            <div><dt>Existing SUBMIT</dt><dd>{BATCH_NEXT4_P3_TARGET.submitCommandId}</dd></div>
            <div><dt>PUBLISH command</dt><dd>{BATCH_NEXT4_P3_TARGET.publishCommandId}</dd></div>
            <div><dt>Frozen scope</dt><dd>{Object.keys(BATCH_NEXT4_P3_TARGET.identities).join(' / ')}</dd></div>

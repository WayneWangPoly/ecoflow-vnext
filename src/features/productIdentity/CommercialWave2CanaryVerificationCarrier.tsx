...
export function CommercialWave2CanaryVerificationCarrier({ role }: Props) {
  ...
  async function runVerification() {
    setBusy(true);
    setReport(null);
    setMessage('Reading caller-authenticated evidence through the P3 verification-only RPC…');
    try {
    ...
    <section className="survey-reconciliation-panel" aria-label="Commercial Promotion Wave 2 P3 verification-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          ...
          <p>Caller-authenticated production reads only. No edit, replay, repair, reconcile, publish, provider, or P4 capability exists here.</p>
        </div>
      </header>
      ...
      <div className="survey-reconciliation-layout">
        ...
        <fieldset className="survey-reconciliation-form" disabled={busy}>
          ...
          <p>The bounded no-argument RPC reads exact identity, mapping, lineage, audit, negative-space, and sentinel evidence. It performs no business write.</p>
        ...
        <fieldset className="survey-reconciliation-form" disabled aria-label="P4 expansion locked">
          ...
          <p>No expansion selection, enablement, command, provider action, caller switch, retirement, or cutover control exists in this carrier.</p>
        </fieldset>
      </div>
      ...
      {report ? (
        <>
          <dl className="survey-reconciliation-evidence survey-reconciliation-note">
            ...
            <div><dt>Provider attribution</dt><dd>{report.providerSentinel.status} · P3A emitted {report.providerSentinel.p3aEmittedProviderTraffic}</dd></div>
            <div><dt>Authority</dt><dd>bounded Commercial identity read only · no production business mutation</dd></div>
          </dl>
          <details>

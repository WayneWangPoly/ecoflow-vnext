export type BatchNext5P2CurrentBatch = {
  batchId: string;
  batchName?: string;
  batchStatus: string;
  revision: number;
...
export type BatchNext5P2EvidenceRow = Record<string, unknown>;

export type BatchNext5P2Evidence = {
  batches: BatchNext5P2EvidenceRow[];
  scopeItems: BatchNext5P2EvidenceRow[];
  reconciliations: BatchNext5P2EvidenceRow[];
  observations: BatchNext5P2EvidenceRow[];
  families: BatchNext5P2EvidenceRow[];
  physicalSkus: BatchNext5P2EvidenceRow[];
  packages: BatchNext5P2EvidenceRow[];
  barcodeBindings: BatchNext5P2EvidenceRow[];
  commercialFamilyLinks: BatchNext5P2EvidenceRow[];
};

export type BatchNext5P2SubmitAcknowledgement = {
  batchId: string;
  batchStatus: string;
  revision: number;
...
export const BATCH_NEXT5_P2_TARGET = {
  protectedMainSha: '71c3a016cdc9e33c0302ea9c9fabecb90bedfcb3',
  ...
  submitNote: 'ECOFLOW-328-BATCH-NEXT5-P2 SUBMIT only; exact eight-SKU DRAFT graph independently verified; PUBLISH, barcode reassignment/retirement, inventory/stocktake/location mutation and provider traffic not authorized.',
  identities: {
    "PSJALLBLACK": {
...
type Identity = (typeof BATCH_NEXT5_P2_TARGET.identities)[keyof typeof BATCH_NEXT5_P2_TARGET.identities];

function hold(detail: string): never {
  throw new Error(`HOLD — ${detail}; SUBMIT was not called.`);
}

function requireCount(rows: BatchNext5P2EvidenceRow[], expected: number, label: string) {
  if (rows.length !== expected) hold(`${label} must contain exactly ${expected} rows`);
}

function expectField(row: BatchNext5P2EvidenceRow, field: string, expected: unknown, label: string) {
...
function findExactly(rows: BatchNext5P2EvidenceRow[], field: string, value: string, label: string) {
  const matches = rows.filter((row) => row[field] === value);
  if (matches.length !== 1) hold(`${label} must contain exactly one row for ${value}`);
  return matches[0];
}

function assertCurrentBatch(currentBatch: BatchNext5P2CurrentBatch | null) {
  if (!currentBatch) hold('current batch authority returned no batch');
  if (currentBatch.batchId !== BATCH_NEXT5_P2_TARGET.batchId) hold('current batch ID mismatch');
  if (currentBatch.batchName !== BATCH_NEXT5_P2_TARGET.batchName) hold('current batch name mismatch');
  if (currentBatch.batchStatus !== 'DRAFT') hold('current batch must be DRAFT');
  if (currentBatch.revision !== BATCH_NEXT5_P2_TARGET.expectedRevision) hold('current batch must be revision 8');
  if (currentBatch.openTasks !== 0 || currentBatch.draftReadyTasks !== 8 || currentBatch.conflictTasks !== 0) {
    hold('current batch tasks must be 0 open / 8 draft-ready / 0 conflict');
  }
  if (!currentBatch.canSubmit) hold('current batch authority returned canSubmit=false');
}

function assertBatchRow(rows: BatchNext5P2EvidenceRow[]) {
  requireCount(rows, 1, 'batch evidence');
  const row = rows[0];
  for (const [field, expected] of Object.entries({
    id: BATCH_NEXT5_P2_TARGET.batchId,
    batch_name: BATCH_NEXT5_P2_TARGET.batchName,
    batch_status: 'DRAFT',
    revision: BATCH_NEXT5_P2_TARGET.expectedRevision,
    start_command_id: BATCH_NEXT5_P2_TARGET.startCommandId,
...
function assertScope(rows: BatchNext5P2EvidenceRow[]) {
  ...
  for (const identity of Object.values(BATCH_NEXT5_P2_TARGET.identities)) {
    ...
    expectField(row, 'batch_id', BATCH_NEXT5_P2_TARGET.batchId, 'scope evidence');
    expectField(row, 'start_command_id', BATCH_NEXT5_P2_TARGET.startCommandId, 'scope evidence');
  }
}

function assertIdentity(evidence: BatchNext5P2Evidence, code: string, identity: Identity) {
  const reconciliation = findExactly(evidence.reconciliations, 'id', identity.reconciliationId, `${code} reconciliation`);
  for (const [field, expected] of Object.entries({
    batch_id: BATCH_NEXT5_P2_TARGET.batchId,
  ...
  for (const [field, expected] of Object.entries({
    ...
    created_in_batch_id: BATCH_NEXT5_P2_TARGET.batchId,
...
export function assertBatchNext5P2ResumeEvidence(
  currentBatch: BatchNext5P2CurrentBatch | null,
  evidence: BatchNext5P2Evidence,
) {
  ...
  ] as const) requireCount(rows, 8, label);

  for (const [code, identity] of Object.entries(BATCH_NEXT5_P2_TARGET.identities)) {
    assertIdentity(evidence, code, identity);
  }
}

export function buildBatchNext5P2SubmitInput() {
  return {
    batchId: BATCH_NEXT5_P2_TARGET.batchId,
    expectedRevision: BATCH_NEXT5_P2_TARGET.expectedRevision,
    commandId: BATCH_NEXT5_P2_TARGET.submitCommandId,
    note: BATCH_NEXT5_P2_TARGET.submitNote,
  };
}

export function assertBatchNext5P2SubmitAcknowledgement(result: BatchNext5P2SubmitAcknowledgement) {
  if (
    result.batchId !== BATCH_NEXT5_P2_TARGET.batchId
    || result.batchStatus !== 'SUBMITTED'
    || result.revision !== 9
    || !['APPLIED', 'REPLAYED'].includes(result.commandStatus)
  ) throw new Error('Batch Next 5 P2 SUBMIT acknowledgement is not the exact SUBMITTED rev9 APPLIED/REPLAYED result.');
}

export function formatBatchNext5P2Failure(error: unknown, commandCrossedBoundary: boolean) {

export type BatchNext4P3CurrentBatch = {
  batchId: string;
  batchName?: string;
  batchStatus: string;
  revision: number;
...
export type BatchNext4P3EvidenceRow = Record<string, unknown>;

export type BatchNext4P3Evidence = {
  batches: BatchNext4P3EvidenceRow[];
  scopeItems: BatchNext4P3EvidenceRow[];
  reconciliations: BatchNext4P3EvidenceRow[];
  observations: BatchNext4P3EvidenceRow[];
  tasks: BatchNext4P3EvidenceRow[];
  families: BatchNext4P3EvidenceRow[];
  physicalSkus: BatchNext4P3EvidenceRow[];
  packages: BatchNext4P3EvidenceRow[];
  barcodeBindings: BatchNext4P3EvidenceRow[];
  commercialFamilyLinks: BatchNext4P3EvidenceRow[];
  publicationAudits: BatchNext4P3EvidenceRow[];
  quantitySentinels: {
    inventoryMovements: BatchNext4P3EvidenceRow[];
    warehouseMovements: BatchNext4P3EvidenceRow[];
    warehouseLocationItems: BatchNext4P3EvidenceRow[];
    inventoryBalances: BatchNext4P3EvidenceRow[];
    stockMovements: BatchNext4P3EvidenceRow[];
  };
};

export type BatchNext4P3PublishAcknowledgement = {
  batchId: string;
  batchStatus: string;
  revision: number;
...
export const BATCH_NEXT4_P3_TARGET = {
  protectedMainSha: '016f8d2b7a412979e67d449da0c19dd7975e1e65',
  ...
  submittedAt: '2026-09-19T10:08:31.309463+00:00',
  publishCommandId: 'be8663c9-cfff-41f1-abb6-fc71a8ca2ffa',
  publishNote: 'ECOFLOW-328-BATCH-NEXT4-P3 PUBLISH only; exact ten-SKU SUBMITTED rev11 graph independently verified; activation-only impact preview; no inventory/stocktake/location mutation, barcode reassignment/retirement or provider authority granted.',
  identities: {
    "BSB42LPLA": {
...
type Identity = (typeof BATCH_NEXT4_P3_TARGET.identities)[keyof typeof BATCH_NEXT4_P3_TARGET.identities];

function hold(detail: string): never {
  throw new Error(`HOLD — ${detail}; PUBLISH was not called.`);
}

function requireCount(rows: BatchNext4P3EvidenceRow[], expected: number, label: string) {
  if (rows.length !== expected) hold(`${label} must contain exactly ${expected} rows`);
}

function expectField(row: BatchNext4P3EvidenceRow, field: string, expected: unknown, label: string) {
...
function findExactly(rows: BatchNext4P3EvidenceRow[], field: string, value: string, label: string) {
  const matches = rows.filter((row) => row[field] === value);
  if (matches.length !== 1) hold(`${label} must contain exactly one row for ${value}`);
  return matches[0];
}

function assertCurrentBatch(currentBatch: BatchNext4P3CurrentBatch | null) {
  const t = BATCH_NEXT4_P3_TARGET;
...
function assertBatch(rows: BatchNext4P3EvidenceRow[], phase: 'PRE' | 'POST') {
  const t = BATCH_NEXT4_P3_TARGET;
...
function assertScope(rows: BatchNext4P3EvidenceRow[]) {
  requireCount(rows, 10, 'scope evidence');
  for (const identity of Object.values(BATCH_NEXT4_P3_TARGET.identities)) {
    const row = findExactly(rows, 'commercial_sku_id', identity.commercialSkuId, 'scope evidence');
    expectField(row, 'batch_id', BATCH_NEXT4_P3_TARGET.batchId, 'scope evidence');
    expectField(row, 'start_command_id', BATCH_NEXT4_P3_TARGET.startCommandId, 'scope evidence');
  }
}

function assertReconciliationAndObservation(evidence: BatchNext4P3Evidence, code: string, identity: Identity) {
  const r = findExactly(evidence.reconciliations, 'id', identity.reconciliationId, `${code} reconciliation`);
  for (const [field, expected] of Object.entries({
    batch_id: BATCH_NEXT4_P3_TARGET.batchId,
...
function assertTasks(rows: BatchNext4P3EvidenceRow[], phase: 'PRE' | 'POST') {
  ...
  for (const identity of Object.values(BATCH_NEXT4_P3_TARGET.identities)) {
    ...
    expectField(row, 'batch_id', BATCH_NEXT4_P3_TARGET.batchId, 'task evidence');
    expectField(row, 'task_status', expectedStatus, 'task evidence');
    expectField(row, 'blocking', true, 'task evidence');
  }
}

function assertCanonicalGraph(evidence: BatchNext4P3Evidence, phase: 'PRE' | 'POST') {
  ...
  for (const [code, identity] of Object.entries(BATCH_NEXT4_P3_TARGET.identities)) {
    const f = findExactly(evidence.families, 'id', identity.familyId, `${code} family`);
    for (const [field, expected] of Object.entries({
      family_code: code,
      family_name: identity.physicalName,
      identity_status: status,
      created_in_batch_id: BATCH_NEXT4_P3_TARGET.batchId,
...
function assertPublicationAudit(rows: BatchNext4P3EvidenceRow[], phase: 'PRE' | 'POST') {
  requireCount(rows, 1, `${phase} publication audit`);
  const row = rows[0];
  for (const [field, expected] of Object.entries({
    batch_id: BATCH_NEXT4_P3_TARGET.batchId,
    batch_name: BATCH_NEXT4_P3_TARGET.batchName,
  ...
  expectTimestamp(row.submitted_at, BATCH_NEXT4_P3_TARGET.submittedAt, `${phase} publication audit submitted_at`);
...
function assertQuantityIsolation(evidence: BatchNext4P3Evidence, phase: 'PRE' | 'POST') {
...
export function assertBatchNext4P3Preflight(
  currentBatch: BatchNext4P3CurrentBatch | null,
  evidence: BatchNext4P3Evidence,
) {
...
export function assertBatchNext4P3Postflight(evidence: BatchNext4P3Evidence) {
...
export function buildBatchNext4P3PublishInput() {
  return {
    batchId: BATCH_NEXT4_P3_TARGET.batchId,
    expectedRevision: BATCH_NEXT4_P3_TARGET.expectedRevision,
    commandId: BATCH_NEXT4_P3_TARGET.publishCommandId,
    note: BATCH_NEXT4_P3_TARGET.publishNote,
  };
}

export function assertBatchNext4P3PublishAcknowledgement(result: BatchNext4P3PublishAcknowledgement) {
  if (
    result.batchId !== BATCH_NEXT4_P3_TARGET.batchId
  ...
  ) throw new Error('Batch Next 4 P3 PUBLISH acknowledgement is not the exact PUBLISHED rev12 / 10-10-10-10 result.');
}

export function formatBatchNext4P3Failure(error: unknown, commandCrossedBoundary: boolean) {

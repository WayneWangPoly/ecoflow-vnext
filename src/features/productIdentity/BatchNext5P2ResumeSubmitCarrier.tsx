...
import { readBatchNext5P2ResumeEvidence } from '@/data/repositories/batchNext5P2ResumeEvidence';
import {
...
import {
  ...
  assertBatchNext5P2ResumeEvidence,
  assertBatchNext5P2SubmitAcknowledgement,
  buildBatchNext5P2SubmitInput,
  formatBatchNext5P2Failure,
  type BatchNext5P2Evidence,
} from './batchNext5P2ResumeSubmitContract';

type Props = { role: Role; onChanged: () => void };

export function BatchNext5P2ResumeSubmitCarrier({ role, onChanged }: Props) {
  ...
  const [evidence, setEvidence] = useState<BatchNext5P2Evidence | null>(null);
  const [result, setResult] = useState<ProductIdentityBatchCommandResult | null>(null);
  const [message, setMessage] = useState('No Batch Next 5 P2 SUBMIT command has been sent.');
  ...
  async function resumeAndSubmit() {
    ...
    setPreflight(null);
    setEvidence(null);
    setMessage('Reading the exact eight-SKU authenticated SUBMIT gate…');
    try {
      const currentBatch = await readCurrentProductIdentityBatch();
      const serverEvidence = await readBatchNext5P2ResumeEvidence();
      assertBatchNext5P2ResumeEvidence(currentBatch, serverEvidence);
      setPreflight(currentBatch);
      setEvidence(serverEvidence);

      const input = buildBatchNext5P2SubmitInput();
      commandCrossedBoundary = true;
      setCommandAttempted(true);
      const acknowledgement = await submitProductIdentityBatch(input);
      assertBatchNext5P2SubmitAcknowledgement(acknowledgement);
      setResult(acknowledgement);
      setMessage('SUBMITTED rev9 — STOP. PUBLISH is not available in this carrier and requires separate authority.');
      onChanged();
    } catch (error) {
      setMessage(formatBatchNext5P2Failure(error, commandCrossedBoundary));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next 5 P2 submit-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · EIGHT-SKU SUBMIT-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next 5 P2 SUBMIT gate</h2>
          <p>
            Frozen to one production batch at DRAFT revision 8. It performs nine authenticated SELECT-only evidence reads,
            verifies the exact eight reconciliation graphs, then makes exactly one incumbent SUBMIT call if every canonical row matches.
          </p>
        </div>
      </header>
      ...
      <div className="survey-reconciliation-layout">
        <fieldset className="survey-reconciliation-form" disabled={busy || commandAttempted || result !== null}>
          ...
          <dl className="survey-reconciliation-evidence survey-reconciliation-note">
            ...
            <div><dt>Frozen scope</dt><dd>{Object.keys(BATCH_NEXT5_P2_TARGET.identities).join(' / ')}</dd></div>
          </dl>
          <button
            type="button"
          ...
          >
            {busy ? 'Reading exact gate and submitting…' : 'Read exact server gate, then SUBMIT eight-SKU batch'}
          </button>
        </fieldset>

...
import { readBatchNextP3ResumeEvidence } from '@/data/repositories/batchNextP3ResumeEvidence';
import {
...
import {
  ...
  assertBatchNextP3Postflight,
  assertBatchNextP3Preflight,
  assertBatchNextP3PublishAcknowledgement,
  buildBatchNextP3PublishInput,
  formatBatchNextP3Failure,
  type BatchNextP3Evidence,
} from './batchNextP3ResumePublishContract';

type Props = { role: Role; onChanged: () => void };

export function BatchNextP3ResumePublishCarrier({ role, onChanged }: Props) {
  ...
  const [preflightEvidence, setPreflightEvidence] = useState<BatchNextP3Evidence | null>(null);
  const [postflightEvidence, setPostflightEvidence] = useState<BatchNextP3Evidence | null>(null);
  const [result, setResult] = useState<ProductIdentityPublishResult | null>(null);
  const [message, setMessage] = useState('No Batch Next P3 PUBLISH command has been sent.');
  ...
  async function resumeAndPublish() {
    ...
    setMessage('Reading the exact authenticated five-SKU PUBLISH gate…');

    try {
      const currentBatch = await readCurrentProductIdentityBatch();
      const serverEvidence = await readBatchNextP3ResumeEvidence('PRE');
      assertBatchNextP3Preflight(currentBatch, serverEvidence);
      setPreflight(currentBatch);
      setPreflightEvidence(serverEvidence);

      const input = buildBatchNextP3PublishInput();
      commandCrossedBoundary = true;
      setCommandAttempted(true);
      const acknowledgement = await publishProductIdentityBatch(input);
      assertBatchNextP3PublishAcknowledgement(acknowledgement);

      const postflight = await readBatchNextP3ResumeEvidence('POST');
      assertBatchNextP3Postflight(postflight);
      setPostflightEvidence(postflight);
      setResult(acknowledgement);
      setMessage('PUBLISHED rev7 with exact 5/5/5/5 acknowledgement and postflight — STOP.');
      onChanged();
    } catch (error) {
      setMessage(formatBatchNextP3Failure(error, commandCrossedBoundary));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next P3 publish-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · FIVE-SKU PUBLISH-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next P3 PUBLISH gate</h2>
          <p>Frozen to one SUBMITTED rev6 production batch. It re-reads the complete canonical DRAFT graph and quantity-isolation sentinels before exactly one incumbent PUBLISH attempt, then performs an authenticated postflight.</p>
        </div>
      </header>
      ...
      <div className="survey-reconciliation-layout">
        <fieldset className="survey-reconciliation-form" disabled={busy || commandAttempted || result !== null}>
          ...
          <dl className="survey-reconciliation-evidence survey-reconciliation-note">
            <div><dt>Protected main</dt><dd>{BATCH_NEXT_P3_TARGET.protectedMainSha}</dd></div>
            <div><dt>Batch ID</dt><dd>{BATCH_NEXT_P3_TARGET.batchId}</dd></div>
            <div><dt>Required state</dt><dd>SUBMITTED revision 6</dd></div>
            <div><dt>Existing SUBMIT</dt><dd>{BATCH_NEXT_P3_TARGET.submitCommandId}</dd></div>
            <div><dt>PUBLISH command</dt><dd>{BATCH_NEXT_P3_TARGET.publishCommandId}</dd></div>
            <div><dt>Frozen scope</dt><dd>{Object.keys(BATCH_NEXT_P3_TARGET.identities).join(' / ')}</dd></div>
            <div><dt>Expected result</dt><dd>PUBLISHED revision 7 · 5/5/5/5</dd></div>
          </dl>
          <button type="button" className="survey-reconciliation-primary" onClick={() => void resumeAndPublish()}>
            {busy ? 'Reading exact gate and publishing…' : 'Read exact server gate, then PUBLISH five-SKU batch'}
          </button>
        </fieldset>

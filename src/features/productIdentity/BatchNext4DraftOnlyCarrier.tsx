...
import { readBatchNext4ResumeEvidence } from '@/data/repositories/batchNext4DraftResumeEvidence';
import {
  ECOFLOW_328_BATCH_NEXT4_DRAFT_TARGET,
  buildBatchNext4ReconcileInput,
  buildBatchNext4StartInput,
  validateBatchNext4Queue,
  type BatchNext4QueueEvidence,
} from './batchNext4DraftOnlyContract';
...
function assertExactResumeBatch(
  evidence: Awaited<ReturnType<typeof readBatchNext4ResumeEvidence>>,
  batchId: string,
  queueEvidence: BatchNext4QueueEvidence[],
) {
  ...
  ) throw new Error('Existing DRAFT batch does not match the frozen Batch Next 4 authority.');
...
export function BatchNext4DraftOnlyCarrier({ role, onChanged }: Props) {
  const authorized = role === 'owner' || role === 'admin';
  const target = ECOFLOW_328_BATCH_NEXT4_DRAFT_TARGET;
  const [busy, setBusy] = useState<BusyState>(null);
  const [message, setMessage] = useState('No Batch Next 4 command has been sent.');
  const [mode, setMode] = useState<Mode>(null);
  const [batchId, setBatchId] = useState<string | null>(null);
  const [batchRevision, setBatchRevision] = useState<number | null>(null);
  const [preflightEvidence, setPreflightEvidence] = useState<BatchNext4QueueEvidence[]>([]);
  ...
  async function runPreflight() {
    ...
    try {
      ...
      if (currentBatch?.batchStatus === 'SUBMITTED') {
        throw new Error(`Existing Product Identity batch ${currentBatch.batchId} is SUBMITTED; stop before Batch Next 4.`);
      }

      if (currentBatch?.batchStatus === 'DRAFT') {
        const queueEvidence = validateBatchNext4Queue(queue, true);
        const resumeEvidence = await readBatchNext4ResumeEvidence(currentBatch.batchId);
        ...
        setMessage(
          `Preflight PASS / RESUME: exact authenticated ten-SKU batch ${currentBatch.batchId} is DRAFT rev${currentBatch.revision}; ${completedCodes.size}/10 reconciliations are exact DRAFTED provenance.`,
        );
        return;
      }

      const queueEvidence = validateBatchNext4Queue(queue, false);
      if (queueEvidence.some((item) => item.alreadyDrafted)) {
        throw new Error('Unexpected DRAFT reconciliation without an open exact Batch Next 4 batch.');
      }
      setPreflightEvidence(queueEvidence);
      setMode('NEW');
      setMessage('Preflight PASS / NEW: ten frozen SKU + carton barcode targets resolve to ten unique authenticated READY_TO_RECONCILE Survey observations and no open DRAFT/SUBMITTED Product Identity batch exists.');
    } catch (error) {
  ...
  async function startBatch() {
    ...
    try {
      ...
      const result = await startBoundedProductIdentityBatch(buildBatchNext4StartInput());
      if (
        ...
        throw new Error(
          `Batch Next 4 START returned ${result.batchStatus}/${result.commandStatus} with scope ${result.scopedSkuCount}; stop before reconciliation.`,
        );
      }
  ...
  async function reconcileOne(code: string) {
    ...
    try {
      ...
      const freshQueueEvidence = validateBatchNext4Queue(freshQueue, true);
      const resumeEvidence = await readBatchNext4ResumeEvidence(batchId);
      ...
      const input = buildBatchNext4ReconcileInput(candidate, fresh.row, batchId, confirmed[code] === true);
      crossedBoundary = true;
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next 4 DRAFT-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · AUTHENTICATED TEN-SKU DRAFT-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next 4 Physical Identity DRAFT</h2>
          <p>
            Frozen engineering base <code>{target.protectedMainSha}</code>. Authenticated census found
            {' '}<strong>{target.authenticatedCensus.ready} READY</strong>; the ten highest-reference-quantity eligible SKUs cover
            {' '}<strong>{target.totalReferenceQty} reference cartons</strong>. Survey observation IDs are resolved only inside the authenticated Owner/Admin queue and must remain exact through each write.
          </p>
        </div>
      </header>
      ...
      <div className="survey-reconciliation-form">
        ...
        >
          {busy === 'PREFLIGHT' ? 'Resolving ten authenticated candidates…' : 'Run NEXT4 ten-SKU DRAFT preflight'}
        </button>
      ...
      <div className="survey-reconciliation-form">
        ...
        >
          {busy === 'START' ? 'Starting NEXT4 DRAFT batch…' : 'Start frozen NEXT4 ten-SKU DRAFT batch'}
        </button>
      </div>

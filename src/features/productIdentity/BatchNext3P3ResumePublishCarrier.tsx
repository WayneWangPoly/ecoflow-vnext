...
import { readBatchNext3P3ResumeEvidence } from '@/data/repositories/batchNext3P3ResumeEvidence';
import {
...
import {
  ...
  assertBatchNext3P3Postflight,
  assertBatchNext3P3Preflight,
  assertBatchNext3P3PublishAcknowledgement,
  buildBatchNext3P3PublishInput,
  formatBatchNext3P3Failure,
  type BatchNext3P3Evidence,
} from './batchNext3P3ResumePublishContract';

type Props = { role: Role; onChanged: () => void };

export function BatchNext3P3ResumePublishCarrier({ role, onChanged }: Props) {
  ...
  const [preflightEvidence, setPreflightEvidence] = useState<BatchNext3P3Evidence | null>(null);
  const [postflightEvidence, setPostflightEvidence] = useState<BatchNext3P3Evidence | null>(null);
  const [result, setResult] = useState<ProductIdentityPublishResult | null>(null);
  const [message, setMessage] = useState('No Batch Next 3 P3 PUBLISH command has been sent.');
  ...
  async function resumeAndPublish() {
    ...
    try {
      ...
      const serverEvidence = await readBatchNext3P3ResumeEvidence('PRE');
      assertBatchNext3P3Preflight(currentBatch, serverEvidence);
      setPreflight(currentBatch);
      setPreflightEvidence(serverEvidence);

      const input = buildBatchNext3P3PublishInput();
      commandCrossedBoundary = true;
      setCommandAttempted(true);
      const acknowledgement = await publishProductIdentityBatch(input);
      assertBatchNext3P3PublishAcknowledgement(acknowledgement);

      const postflight = await readBatchNext3P3ResumeEvidence('POST');
      assertBatchNext3P3Postflight(postflight);
      ...
      setMessage(formatBatchNext3P3Failure(error, commandCrossedBoundary));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next 3 P3 publish-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · TEN-SKU PUBLISH-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next 3 P3 PUBLISH gate</h2>
      ...
      <div className="survey-reconciliation-layout">
        <fieldset className="survey-reconciliation-form" disabled={busy || commandAttempted || result !== null}>
          ...
          <dl className="survey-reconciliation-evidence survey-reconciliation-note">
            <div><dt>Protected main</dt><dd>{BATCH_NEXT3_P3_TARGET.protectedMainSha}</dd></div>
            <div><dt>Batch ID</dt><dd>{BATCH_NEXT3_P3_TARGET.batchId}</dd></div>
            <div><dt>Required state</dt><dd>SUBMITTED revision 11</dd></div>
            <div><dt>Existing SUBMIT</dt><dd>{BATCH_NEXT3_P3_TARGET.submitCommandId}</dd></div>
            <div><dt>PUBLISH command</dt><dd>{BATCH_NEXT3_P3_TARGET.publishCommandId}</dd></div>
            <div><dt>Frozen scope</dt><dd>{Object.keys(BATCH_NEXT3_P3_TARGET.identities).join(' / ')}</dd></div>

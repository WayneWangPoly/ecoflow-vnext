...
import { readBatchNext2ResumeEvidence } from '@/data/repositories/batchNext2DraftResumeEvidence';
import {
  ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET,
  buildBatchNext2ReconcileInput,
  buildBatchNext2StartInput,
  validateBatchNext2Queue,
  type BatchNext2QueueEvidence,
} from './batchNext2DraftOnlyContract';
...
function assertExactResumeBatch(
  evidence: Awaited<ReturnType<typeof readBatchNext2ResumeEvidence>>,
  batchId: string,
) {
  ...
  ) throw new Error('Existing DRAFT batch does not match the frozen Batch Next 2 authority.');
...
export function BatchNext2DraftOnlyCarrier({ role, onChanged }: Props) {
  const authorized = role === 'owner' || role === 'admin';
  const target = ECOFLOW_328_BATCH_NEXT2_DRAFT_TARGET;
  const [busy, setBusy] = useState<BusyState>(null);
  const [message, setMessage] = useState('No Batch Next 2 command has been sent.');
  const [mode, setMode] = useState<Mode>(null);
  const [batchId, setBatchId] = useState<string | null>(null);
  const [batchRevision, setBatchRevision] = useState<number | null>(null);
  const [preflightEvidence, setPreflightEvidence] = useState<BatchNext2QueueEvidence[]>([]);
  ...
  async function runPreflight() {
    ...
    try {
      ...
      if (currentBatch?.batchStatus === 'SUBMITTED') {
        throw new Error(`Existing Product Identity batch ${currentBatch.batchId} is SUBMITTED; stop before Batch Next 2.`);
      }

      if (currentBatch?.batchStatus === 'DRAFT') {
        const resumeEvidence = await readBatchNext2ResumeEvidence(currentBatch.batchId);
        assertExactResumeBatch(resumeEvidence, currentBatch.batchId);
        const evidence = validateBatchNext2Queue(queue, true);
      ...
      const evidence = validateBatchNext2Queue(queue, false);
      if (evidence.some((item) => item.alreadyDrafted)) {
        throw new Error('Unexpected existing DRAFT reconciliation without an open exact Batch Next 2 batch.');
      }
      setPreflightEvidence(evidence);
      setMode('NEW');
  ...
  async function startBatch() {
    ...
    setMessage('');
    try {
      const result = await startBoundedProductIdentityBatch(buildBatchNext2StartInput());
      if (
        ...
        throw new Error(
          `Batch Next 2 START returned ${result.batchStatus}/${result.commandStatus} with scope ${result.scopedSkuCount}; stop before reconciliation.`,
        );
      }
  ...
  async function reconcileOne(code: string) {
    ...
    setMessage('');
    try {
      const input = buildBatchNext2ReconcileInput(candidate, batchId, confirmed[code] === true);
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next 2 DRAFT-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · TEN-SKU DRAFT-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next 2 Physical Identity DRAFT</h2>
          <p>

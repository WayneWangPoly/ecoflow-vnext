...
import { readBatchNext3P2ResumeEvidence } from '@/data/repositories/batchNext3P2ResumeEvidence';
import {
...
import {
  ...
  assertBatchNext3P2ResumeEvidence,
  assertBatchNext3P2SubmitAcknowledgement,
  buildBatchNext3P2SubmitInput,
  formatBatchNext3P2Failure,
  type BatchNext3P2Evidence,
} from './batchNext3P2ResumeSubmitContract';

type Props = { role: Role; onChanged: () => void };

export function BatchNext3P2ResumeSubmitCarrier({ role, onChanged }: Props) {
  ...
  const [evidence, setEvidence] = useState<BatchNext3P2Evidence | null>(null);
  const [result, setResult] = useState<ProductIdentityBatchCommandResult | null>(null);
  const [message, setMessage] = useState('No Batch Next 3 P2 SUBMIT command has been sent.');
  ...
  async function resumeAndSubmit() {
    ...
    try {
      ...
      const serverEvidence = await readBatchNext3P2ResumeEvidence();
      assertBatchNext3P2ResumeEvidence(currentBatch, serverEvidence);
      setPreflight(currentBatch);
      setEvidence(serverEvidence);

      const input = buildBatchNext3P2SubmitInput();
      commandCrossedBoundary = true;
      setCommandAttempted(true);
      const acknowledgement = await submitProductIdentityBatch(input);
      assertBatchNext3P2SubmitAcknowledgement(acknowledgement);
      ...
      setMessage(formatBatchNext3P2Failure(error, commandCrossedBoundary));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="survey-reconciliation-panel" aria-label="ECOFLOW-328 Batch Next 3 P2 submit-only carrier">
      <header className="survey-reconciliation-header">
        <div>
          <span>OWNER / ADMIN · TEN-SKU SUBMIT-ONLY AUTHORITY</span>
          <h2>ECOFLOW-328 · Batch Next 3 P2 SUBMIT gate</h2>
          <p>
      ...
      <div className="survey-reconciliation-layout">
        <fieldset className="survey-reconciliation-form" disabled={busy || commandAttempted || result !== null}>
          ...
          <dl className="survey-reconciliation-evidence survey-reconciliation-note">
            <div><dt>Engineering base</dt><dd>{BATCH_NEXT3_P2_TARGET.protectedMainSha}</dd></div>
            <div><dt>Batch ID</dt><dd>{BATCH_NEXT3_P2_TARGET.batchId}</dd></div>
            <div><dt>Required state</dt><dd>DRAFT revision 10</dd></div>
            <div><dt>Required tasks</dt><dd>0 open / 10 draft-ready / 0 conflict</dd></div>
            <div><dt>Expected revision</dt><dd>{BATCH_NEXT3_P2_TARGET.expectedRevision}</dd></div>
            <div><dt>SUBMIT command</dt><dd>{BATCH_NEXT3_P2_TARGET.submitCommandId}</dd></div>
            <div><dt>Frozen scope</dt><dd>{Object.keys(BATCH_NEXT3_P2_TARGET.identities).join(' / ')}</dd></div>
          </dl>
          <button
            type="button"

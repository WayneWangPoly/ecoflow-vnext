...
export const IMAGE_COPY_WINDOW_37 = {
  commandId: 'f6e21021-7249-4aff-80bd-0164e4fd9604',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_37.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow37(
  ...
  const [{ data: predecessor, error: predecessorError }, { data: rights, error: rightsError }] = await Promise.all([
    supabase.from('ecoflow_unleashed_asset_copy_runs')
      ...
      .eq('command_id', '0da2ed8a-a4e5-4544-b295-9308f9a8a4d3').single(),
  ...
  if (!predecessor || predecessor.id !== '7d464f58-7305-433b-a618-dcaeb31d820d'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 3376311
      || predecessor.authorization_id !== '15612d15-f97e-462e-a24f-49889b4668c2') {
    throw new Error('UNLEASHED_IMAGE_COPY_W36_PREDECESSOR_REJECTED');
  }
  if (!rights || rights.id !== '15612d15-f97e-462e-a24f-49889b4668c2'
    ...
    throw new Error('UNLEASHED_IMAGE_COPY_W37_AUTHORIZATION_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_37.commandId,
      limit: IMAGE_COPY_WINDOW_37.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 37 after verified W36 run 7d464f58-7305-433b-a618-dcaeb31d820d: 10 copied, 3376311 bytes; cumulative Storage 350 private objects / 88815566 bytes, 85 PLANNED, 32 terminal BLOCKED and zero claims; revision 2 128 MiB aggregate budget, 2 MiB/object, max 10 and stop for production verification before continuation',
    },
  });

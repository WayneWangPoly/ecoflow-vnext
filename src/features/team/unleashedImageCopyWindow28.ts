...
export const IMAGE_COPY_WINDOW_28 = {
  commandId: '3180af4a-2dd7-475d-b831-ae4e95e527b1',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_28.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow28(
  ...
  const { data: predecessor, error: predecessorError } = await supabase
    ...
    .eq('command_id', '3fe47bbc-3c5d-4898-a58b-9fb3ec5ea76b').single();
  if (predecessorError) throw predecessorError;
  if (!predecessor || predecessor.id !== '1cdfb299-5fca-4325-9ab8-2c03c808ddc7'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 2686818
      || predecessor.authorization_id !== '9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c') {
    throw new Error('UNLEASHED_IMAGE_COPY_W27_PREDECESSOR_REJECTED');
  }

  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_28.commandId,
      limit: IMAGE_COPY_WINDOW_28.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 28 after verified W27 run 1cdfb299-5fca-4325-9ab8-2c03c808ddc7: 10 copied, 2686818 bytes; cumulative Storage 265 private objects / 64473637 bytes, 32 terminal BLOCKED assets and zero claims; limit 10 and stop for production verification before continuation',
    },
  });

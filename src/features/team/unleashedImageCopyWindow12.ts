...
export const IMAGE_COPY_WINDOW_12 = {
  commandId: 'f70d6b49-1c62-456c-b9a7-124572c95313',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_12.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow12(
  ...
  const { data: predecessor, error: predecessorError } = await supabase
    ...
    .eq('command_id', '2c7b2cf0-65f0-49e7-8cf1-4a661580404b').single();
  if (predecessorError) throw predecessorError;
  if (!predecessor || predecessor.id !== '1575c3d7-2348-406b-bf46-61ef8f521779'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 2310821
      || predecessor.authorization_id !== '9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c') {
    throw new Error('UNLEASHED_IMAGE_COPY_W11_PREDECESSOR_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_12.commandId,
      limit: IMAGE_COPY_WINDOW_12.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 12 after verified W11 run 1575c3d7-2348-406b-bf46-61ef8f521779: 10 copied, 2310821 bytes; cumulative Storage 110 private objects / 29807639 bytes, valid provenance and zero claims; limit 10 and stop for production verification before continuation',
    },
  });

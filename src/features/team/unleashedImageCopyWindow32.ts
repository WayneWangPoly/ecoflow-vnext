...
export const IMAGE_COPY_WINDOW_32 = {
  commandId: '4149b23d-a24e-4e31-b0c9-0c26bf4df758',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_32.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow32(
  ...
  const [{ data: predecessor, error: predecessorError }, { data: rights, error: rightsError }] = await Promise.all([
    supabase.from('ecoflow_unleashed_asset_copy_runs')
      ...
      .eq('command_id', '8ca61897-3cda-4909-8515-b6de152e98d2').single(),
  ...
  if (!predecessor || predecessor.id !== '2a857937-ff68-49e8-9f2c-a3b99c5766ea'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 4779765
      || predecessor.authorization_id !== '15612d15-f97e-462e-a24f-49889b4668c2') {
    throw new Error('UNLEASHED_IMAGE_COPY_W31_PREDECESSOR_REJECTED');
  }
  if (!rights || rights.id !== '15612d15-f97e-462e-a24f-49889b4668c2'
    ...
    throw new Error('UNLEASHED_IMAGE_COPY_W32_AUTHORIZATION_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_32.commandId,
      limit: IMAGE_COPY_WINDOW_32.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 32 after verified W31 run 2a857937-ff68-49e8-9f2c-a3b99c5766ea: 10 copied, 4779765 bytes; cumulative Storage 300 private objects / 76265602 bytes, 135 PLANNED, 32 terminal BLOCKED and zero claims; revision 2 128 MiB aggregate budget, 2 MiB/object, max 10 and stop for production verification before continuation',
    },
  });

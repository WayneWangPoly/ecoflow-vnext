...
export const IMAGE_COPY_WINDOW_18 = {
  commandId: '8a66ae1e-099b-4f1f-97ed-fc7e78b2fb7a',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_18.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow18(
  ...
  const { data: predecessor, error: predecessorError } = await supabase
    ...
    .eq('command_id', '5c806850-c9bb-487a-9114-a4510b08895f').single();
  if (predecessorError) throw predecessorError;
  if (!predecessor || predecessor.id !== '0a1b5176-bc92-4680-8373-a521ab13320e'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 2525381
      || predecessor.authorization_id !== '9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c') {
    throw new Error('UNLEASHED_IMAGE_COPY_W17_PREDECESSOR_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_18.commandId,
      limit: IMAGE_COPY_WINDOW_18.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 18 after verified W17 run 0a1b5176-bc92-4680-8373-a521ab13320e: 10 copied, 2525381 bytes; cumulative Storage 170 private objects / 45723797 bytes, valid provenance and zero claims; limit 10 and stop for production verification before continuation',
    },
  });

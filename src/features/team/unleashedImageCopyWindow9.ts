...
export const IMAGE_COPY_WINDOW_9 = {
  commandId: 'e4096248-3cc6-47e2-b54a-823f69023d32',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_9.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow9(
  ...
  const { data: predecessor, error: predecessorError } = await supabase
    ...
    .eq('command_id', 'd8706a42-cb33-4a69-ac38-0e00ed1c7690').single();
  if (predecessorError) throw predecessorError;
  if (!predecessor || predecessor.id !== 'e4dc80fd-9082-46b0-9d31-51a441d867a3'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 1952538
      || predecessor.authorization_id !== '9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c') {
    throw new Error('UNLEASHED_IMAGE_COPY_W8_PREDECESSOR_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_9.commandId,
      limit: IMAGE_COPY_WINDOW_9.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 9 after verified W8 run e4dc80fd-9082-46b0-9d31-51a441d867a3: 10 copied, 1952538 bytes; cumulative Storage 80 private objects / 23858501 bytes, valid provenance and zero claims; limit 10 and stop for production verification before continuation',
    },
  });

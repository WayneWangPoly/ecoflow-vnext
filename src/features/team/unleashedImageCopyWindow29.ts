...
export const IMAGE_COPY_WINDOW_29 = {
  commandId: '4d731c72-ba5b-4292-8407-ae0f641a0391',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_29.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow29(
  ...
  const { data: predecessor, error: predecessorError } = await supabase
    ...
    .eq('command_id', '3180af4a-2dd7-475d-b831-ae4e95e527b1').single();
  if (predecessorError) throw predecessorError;
  if (!predecessor || predecessor.id !== 'cc8d4398-b5ad-40aa-8076-fec62e65f848'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 1247163
      || predecessor.authorization_id !== '9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c') {
    throw new Error('UNLEASHED_IMAGE_COPY_W28_PREDECESSOR_REJECTED');
  }

  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_29.commandId,
      limit: IMAGE_COPY_WINDOW_29.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 29 after verified W28 run cc8d4398-b5ad-40aa-8076-fec62e65f848: 10 copied, 1247163 bytes; cumulative Storage 275 private objects / 65720800 bytes, 32 terminal BLOCKED assets and zero claims; limit 10 and stop for production verification before continuation',
    },
  });

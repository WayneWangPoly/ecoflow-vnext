...
export const IMAGE_COPY_WINDOW_7 = {
  commandId: 'd3dfda63-3b2a-4f02-900d-9bc3ee659725',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_7.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow7(
  ...
  const { data: predecessor, error: predecessorError } = await supabase
    ...
    .eq('command_id', '97c61ec6-194e-42f0-bc77-4151020ebe4f').single();
  if (predecessorError) throw predecessorError;
  if (!predecessor || predecessor.id !== 'a2bc3cc4-602f-4434-a653-f23bb16133a0'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 1735224
      || predecessor.authorization_id !== '9719f6ff-f1bf-4b3d-ae45-02bfca8a2f9c') {
    throw new Error('UNLEASHED_IMAGE_COPY_W6_PREDECESSOR_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_7.commandId,
      limit: IMAGE_COPY_WINDOW_7.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 7 after verified W6 run a2bc3cc4-602f-4434-a653-f23bb16133a0: 10 copied, 1735224 bytes; cumulative Storage 60 private objects / 19728971 bytes, valid provenance and zero claims; limit 10 and stop for production verification before continuation',
    },
  });

...
export const IMAGE_COPY_WINDOW_33 = {
  commandId: '2a4b5e7e-662e-4d76-a682-3dfc70ccd2e2',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_33.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow33(
  ...
  const [{ data: predecessor, error: predecessorError }, { data: rights, error: rightsError }] = await Promise.all([
    supabase.from('ecoflow_unleashed_asset_copy_runs')
      ...
      .eq('command_id', '4149b23d-a24e-4e31-b0c9-0c26bf4df758').single(),
  ...
  if (!predecessor || predecessor.id !== '18fc4e2e-242a-4512-a50f-c49423bcc038'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 4143423
      || predecessor.authorization_id !== '15612d15-f97e-462e-a24f-49889b4668c2') {
    throw new Error('UNLEASHED_IMAGE_COPY_W32_PREDECESSOR_REJECTED');
  }
  if (!rights || rights.id !== '15612d15-f97e-462e-a24f-49889b4668c2'
    ...
    throw new Error('UNLEASHED_IMAGE_COPY_W33_AUTHORIZATION_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_33.commandId,
      limit: IMAGE_COPY_WINDOW_33.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 33 after verified W32 run 18fc4e2e-242a-4512-a50f-c49423bcc038: 10 copied, 4143423 bytes; cumulative Storage 310 private objects / 80409025 bytes, 125 PLANNED, 32 terminal BLOCKED and zero claims; revision 2 128 MiB aggregate budget, 2 MiB/object, max 10 and stop for production verification before continuation',
    },
  });

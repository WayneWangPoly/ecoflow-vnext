...
export const IMAGE_COPY_WINDOW_36 = {
  commandId: '0da2ed8a-a4e5-4544-b295-9308f9a8a4d3',
  limit: 10,
} as const;
...
function isBoundedWindowResult(value: unknown): value is AuthorizedImageCopyWindowResult {
  ...
  if ((row.assetsPlanned as number) > IMAGE_COPY_WINDOW_36.limit) return false;
  if ((row.assetsCopied as number) + (row.assetsReused as number) + (row.assetsFailed as number) !== row.assetsPlanned) return false;
  return true;
}

export async function runAuthorizedImageCopyWindow36(
  ...
  const [{ data: predecessor, error: predecessorError }, { data: rights, error: rightsError }] = await Promise.all([
    supabase.from('ecoflow_unleashed_asset_copy_runs')
      ...
      .eq('command_id', 'abfea975-60f5-4fed-9dd0-1315e69a2fba').single(),
  ...
  if (!predecessor || predecessor.id !== '8756d3be-a8f3-4408-adaf-18f7e9c0605e'
      || predecessor.status !== 'SUCCEEDED' || predecessor.assets_planned !== 10
      || predecessor.assets_copied !== 10 || predecessor.assets_reused !== 0
      || predecessor.assets_failed !== 0 || predecessor.bytes_copied !== 1738402
      || predecessor.authorization_id !== '15612d15-f97e-462e-a24f-49889b4668c2') {
    throw new Error('UNLEASHED_IMAGE_COPY_W35_PREDECESSOR_REJECTED');
  }
  if (!rights || rights.id !== '15612d15-f97e-462e-a24f-49889b4668c2'
    ...
    throw new Error('UNLEASHED_IMAGE_COPY_W36_AUTHORIZATION_REJECTED');
  }
  const { data, error } = await supabase.functions.invoke('trigger-unleashed-master-migration', {
    body: {
      mode: 'COPY_IMAGES',
      commandId: IMAGE_COPY_WINDOW_36.commandId,
      limit: IMAGE_COPY_WINDOW_36.limit,
      reason: '#338 authorized bounded COPY_IMAGES window 36 after verified W35 run 8756d3be-a8f3-4408-adaf-18f7e9c0605e: 10 copied, 1738402 bytes; cumulative Storage 340 private objects / 85439255 bytes, 95 PLANNED, 32 terminal BLOCKED and zero claims; revision 2 128 MiB aggregate budget, 2 MiB/object, max 10 and stop for production verification before continuation',
    },
  });

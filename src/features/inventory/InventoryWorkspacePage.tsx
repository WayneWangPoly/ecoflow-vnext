...
export function InventoryWorkspacePage({
  ...
  const filteredStock = useMemo(() => {
    ...
    return sortStock(
      stock.filter((row) => {
        ...
        return [row.sku, row.name, row.location, row.source]
  ...
  const filteredCatalog = useMemo(() => {
    ...
    return sortCatalog(
      catalog.filter((row) => {
        ...
        if (state.filter !== 'all' && state.filter !== 'hidden') return true;
        if (!needle) return true;
        return [row.sku, row.name, row.category, row.unit, row.source]
          .some((value) => String(value || '').toLowerCase().includes(needle));
      }),
      state.sort,
    );
  ...
  const activeRows = state.tab === 'stock' ? filteredStock : filteredCatalog;
  ...
  const notice = loadError
    ...
    : healthNotice
      ? `Inventory workspace is degraded: ${healthNotice}`
      : '';
  ...
  return (
    <NativeWorkspaceFrame
      ...
      actions={(
        <>
          <a className="soft-button" href="/warehouse-map">Warehouse map</a>
    ...
    >
      ...
      <section className="panel native-workspace-toolbar">
        <label>
          <span>Search</span>
          <input value={state.search} placeholder="SKU, product, location, category or source" onChange={(event) => update({ search: event.target.value }, { replace: true })} />
        </label>
        <label>
          <span>Filter</span>
          {state.tab === 'stock' ? (
            <select value={state.filter} onChange={(event) => update({ filter: event.target.value })}>
              ...
              <option value="unlocated">No verified location</option>
            </select>
          ) : (
            <select value={state.filter === 'hidden' ? 'hidden' : 'all'} onChange={(event) => update({ filter: event.target.value })}>
        ...
        <label>
          ...
          <select value={state.sort} onChange={(event) => update({ sort: event.target.value })}>
            ...
            {state.tab === 'stock' ? <option value="location">Location</option> : null}
            {state.tab === 'stock' ? <option value="on-hand">On hand</option> : null}
            {state.tab === 'stock' ? <option value="reserved">Reserved</option> : null}
            <option value="signal">Status</option>
            {state.tab === 'catalog' ? <option value="price">Base price</option> : null}
          </select>
        </label>
        <label>
          <span>Page size</span>
      ...
      {available && !activeRows.length ? <NativeWorkspaceEmpty title="No inventory records match this view" detail="Change the query or filters. Missing records are not represented as zero stock." /> : null}

      {available && activeRows.length && state.tab === 'stock' ? (
        <section className="panel">
          ...
          <div className="table-like native-workspace-table">
            ...
            {(page.rows as StockRow[]).map((row) => {
              const signal = stockSignal(row);
              return (
                <div className="table-row" key={`${row.sku}-${row.location}`}>
                  <span><strong><a href={`/inventory/physical/${encodeURIComponent(row.sku)}?tab=stock`}>{row.sku}</a></strong><small>{row.name}</small></span>
                  <span><strong>{row.location || 'UNVERIFIED'}</strong><small>{row.location ? <a href={`/warehouse-map?location=${encodeURIComponent(row.location)}`}>Open map</a> : 'Assign through a warehouse transaction'}</small></span>
                  <span><strong>{row.onHand}</strong><small>{row.source || 'live stock view'}</small></span>
                  <span>{row.reserved}</span>
                  <span>{row.reorderPoint}</span>
                  <span><span className={`pill pill-${signalTone(signal)}`}>{signal}</span></span>
                </div>
              );
            })}
          </div>
      ...
      {available && activeRows.length && state.tab === 'catalog' ? (
        <section className="panel">
          ...
          <div className="table-like native-workspace-table">
            ...
            {(page.rows as CatalogRow[]).map((row) => (
              <div className="table-row" key={`${row.source}-${row.id}`}>
                <span><strong><a href={`/inventory/commercial/${encodeURIComponent(row.id)}?tab=catalog`}>{row.sku}</a></strong><small>{row.category || 'Uncategorised'}</small></span>
                <span><strong>{row.name}</strong><small>{Object.keys(row.tierPrices || {}).length} tier prices</small></span>
                <span><span className="pill pill-blue">{sourceLabel(row.source)}</span></span>
                <span>{row.unit || 'Not set'}</span>
                <span>{row.displayPrice || row.basePrice.toLocaleString('en-AU', { style: 'currency', currency: 'AUD' })}</span>
                <span><span className={`pill pill-${row.visible ? 'good' : 'warn'}`}>{row.visible ? 'VISIBLE' : 'HIDDEN'}</span></span>
              </div>
            ))}
          </div>
          <NativePager page={page.page} totalPages={page.totalPages} totalRows={page.totalRows} onPage={(next) => update({ page: next }, { preservePage: true })} />

...
const dashboardBase = buildEcoFlowData();
...
function operationalError(error: unknown) {
  ...
  if (normalized.includes('current transaction is aborted') || normalized.includes('transaction block') || normalized.includes('25p02')) {
    return 'Live operations could not finish loading because the database session was interrupted. Refresh in a moment; if it continues, an administrator should check the first database error in the Supabase logs.';
  }
  if (normalized.includes('postgres') || normalized.includes('database') || normalized.includes('supabase')) {
    return 'Live operations are temporarily unavailable. No sample figures are being shown. Refresh in a moment or contact an administrator.';
  }
  return raw || 'Live operations are temporarily unavailable.';
}
...
function lineSummary(order: ImportedOrder) {
  ...
  const parts = shown.map((line) => {
    const baseUnit = line.unit === 'sleeve' ? 'sleeve' : 'carton';
    const unit = line.qty === 1 ? baseUnit : `${baseUnit}s`;
    return `${line.sku} × ${line.qty} ${unit}`;
  });
  const remaining = Math.max(0, order.lines.length - shown.length);
  if (remaining) parts.push(`+${remaining} more line${remaining === 1 ? '' : 's'}`);
  return parts.join(' · ');
}

function queueBadges(order: ImportedOrder): QueueBadge[] {
  const raw: QueueBadge[] = [
    { label: title(order.status), tone: orderStatusTone(order.status) },
    { label: title(order.syncStatus), tone: syncTone(order.syncStatus) },
  ];
  if (order.releaseGateStatus) raw.push({ label: title(order.releaseGateStatus), tone: releaseTone(order.releaseGateStatus) });
      ...
      <div className="owner-command-order-main">
        <div className="owner-command-order-title">
          ...
          <div className="owner-command-order-pills">
            {queueBadges(order).map((badge) => <Pill key={badge.label} tone={badge.tone}>{badge.label}</Pill>)}
          </div>
        </div>
...
function openOrdersWorkspace() {
  const buttons = Array.from(document.querySelectorAll<HTMLButtonElement>('.sidebar-nav button, .desktop-mobile-nav button'));
  buttons.find((button) => button.textContent?.trim() === 'Orders')?.click();
}
...
function AttentionRow({ row }: { row: OwnerCommandAttentionRow }) {
  return (
    <article className="owner-command-attention-row">
      ...
      <Pill tone={signalTone(row.signal)}>{row.area || 'Operations'} · {title(row.signal)}</Pill>
    </article>
  );
}
...
function LoadingDashboard() {
  return (
    <section className="owner-command-loading" aria-live="polite">
      ...
      <div><strong>Loading live operations</strong><span>EcoFlow is waiting for one consistent Supabase snapshot before showing totals.</span></div>
    </section>
  );
}
...
function CommandContent({ role }: { role: string }) {
  ...
  async function reload() {
    ...
    try {
      ...
      const nextData = applySupabaseOrdermentumViews(dashboardBase, views);
      setData(nextData);
      setLoadedAt(new Date().toISOString());

      const [kpiResult, attentionResult] = await Promise.allSettled([
        loadOwnerCommandKpis(),
        loadOwnerCommandAttention(),
      ]);
  ...
  const view = useMemo(() => {
    ...
    const latestOrderChange = data.orders
      .map((order) => order.lastSeenAt)
      .filter(Boolean)
      .sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0];
    const warningCount = data.dataQuality.filter((item) => item.severity === 'warn' || item.severity === 'danger').length + data.syncBatch.failed;
  ...
  if (!data || !view) {
    return (
      <section className="owner-command-unavailable" role="alert">
        <div><strong>Live dashboard unavailable</strong><span>{error || 'EcoFlow could not load a consistent live snapshot.'}</span></div>
        <button type="button" onClick={() => void reload()}>Retry live data</button>
      </section>
    );
  }
    ...
    <section className="owner-command-shell">
      <section className="owner-command-hero">
        <div className="owner-command-hero-copy">
          <span>{role === 'ACCOUNT' ? 'ACCOUNTS CONTROL' : 'ECOFLOW CONTROL ROOM'}</span>
          <h1>Build the supply chain behind a cleaner food future.</h1>
          <p>{subtitle}</p>
          <small>{data.businessDay.label} · cutoff {data.businessDay.cutoffTime} · live Supabase workflow</small>
        </div>
        <div className="owner-command-hero-metrics">
          <Metric label="New today" value={view.count('newToday')} helper="First seen today" tone="good" />
          <Metric label="Updated today" value={view.count('updatedToday')} helper="Changed today" tone="blue" />
          <Metric label="Active orders" value={view.activeOrders} helper={`${data.orders.length} in active view`} tone="warn" />
          <Metric label="Open AR" value={money(view.openAr)} helper="Outstanding balance" tone="neutral" />
        </div>
      </section>
      ...
      <section className="owner-command-syncbar">
        <div className="owner-command-sync-summary">
          ...
          <div><strong>Sync control</strong><span>Last successful sync {formatDateTime(data.syncBatch.completedAt)}</span></div>
          <Pill tone={data.syncBatch.status === 'SUCCESS' ? 'good' : 'warn'}>{data.syncBatch.status}</Pill>
        </div>
        <div className="owner-command-sync-metrics">
          ...
          <div className={view.warningCount ? 'is-warning' : ''}><strong>{view.warningCount}</strong><span>Warnings</span></div>
        </div>
        <div className="owner-command-sync-actions">
          <small>Latest order change {view.latestOrderChange ? formatDateTime(view.latestOrderChange) : '—'}<br />View refreshed {formatDateTime(loadedAt)}</small>
          <button type="button" disabled={loading} onClick={() => void reload()}>{loading ? 'Refreshing…' : 'Refresh live data'}</button>
        </div>
      </section>
      ...
      <section className="owner-command-workspace">
        <section className="owner-command-panel owner-command-queue-panel">
          <header className="owner-command-panel-header">
            <div><h2>Daily control queue</h2><p>Changes and blockers only; long order lines are collapsed for fast review.</p></div>
            <div className="owner-command-header-actions"><Pill tone={view.queue.length ? 'warn' : 'good'}>{view.queue.length} shown</Pill><button type="button" onClick={openOrdersWorkspace}>View all orders</button></div>
          </header>
          <div className="owner-command-order-list">
            {view.queue.map((order) => <OrderQueueRow key={order.id} order={order} />)}
        ...
        <aside className="owner-command-rail">
          <section className="owner-command-panel owner-command-rail-card">
            <header className="owner-command-panel-header"><div><h3>Operational buckets</h3><p>{data.businessDay.label}</p></div></header>
            <div className="owner-command-buckets">
              {view.bucketCounts.filter((bucket) => ['exceptions', 'newToday', 'updatedToday', 'dueToday', 'carryOver'].includes(bucket.key)).map((bucket) => (
          ...
          <section className="owner-command-panel owner-command-rail-card">
            <header className="owner-command-panel-header"><div><h3>Business pulse</h3><p>Best available owner signals.</p></div></header>
            <div className="owner-command-pulse-list">
              ...
              <article><strong>{num(kpis?.barcode_attention_lines)}</strong><span>Barcode attention lines</span></article>
              <article><strong>{num(kpis?.reorder_pressure_rows)}</strong><span>Reorder pressure rows</span></article>
            </div>
          </section>

          <section className="owner-command-panel owner-command-rail-card">
            <header className="owner-command-panel-header"><div><h3>Priority attention</h3><p>Highest-value actions across the platform.</p></div></header>
            <div className="owner-command-attention-list">
              {attention.slice(0, 5).map((row, index) => <AttentionRow key={`${row.area}-${row.reference_id}-${index}`} row={row} />)}

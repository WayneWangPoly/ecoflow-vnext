...
function statusLabel(status: TrackerStatus, active: boolean, lastSharedAt: string) {
  ...
  if (lastSharedAt) return `Sharing · ${new Date(lastSharedAt).toLocaleTimeString('en-AU', { hour: '2-digit', minute: '2-digit' })}`;
  return 'Sharing every 3 min';
}
...
export function DriverLocationTracker() {
  ...
  const share = useCallback(async (source: DriverLocationSource) => {
    ...
    try {
      ...
      await recordDriverLocationSample({
        ...
        metadata: { visibility: document.visibilityState, userAgent: navigator.userAgent.slice(0, 180) },
      });
  ...
  useEffect(() => {
    ...
    if (snapshot.eventKey && snapshot.eventSource && lastEventAttemptRef.current !== snapshot.eventKey) {
      lastEventAttemptRef.current = snapshot.eventKey;
      void share(snapshot.eventSource);
      return;
    }

    if (snapshot.active && document.visibilityState === 'visible') {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const current = latestSnapshot();
      if (!current?.active || document.visibilityState !== 'visible') return;

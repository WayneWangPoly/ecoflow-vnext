...
import {
  ...
  type BatchNextP3EvidenceRow,
} from '@/features/productIdentity/batchNextP3ResumePublishContract';
...
function evidenceRows(data: unknown, label: string): BatchNextP3EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next P3 ${label} read returned an invalid row set.`);
  }
  return data as BatchNextP3EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next P3 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNextP3ResumeEvidence(
  phase: 'PRE' | 'POST',
  client?: SupabaseClient | null,
): Promise<BatchNextP3Evidence> {
  const active = activeClient(client);
  const target = BATCH_NEXT_P3_TARGET;
  ...
  const [
    ...
    readRows(`${label} family`, active
      ...
      .eq('created_in_batch_id', batchId).limit(6)),

...
import {
  ...
  type BatchNext2P2EvidenceRow,
} from '@/features/productIdentity/batchNext2P2ResumeSubmitContract';
...
function evidenceRows(data: unknown, label: string): BatchNext2P2EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next 2 P2 ${label} read returned an invalid row set.`);
  }
  return data as BatchNext2P2EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next 2 P2 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNext2P2ResumeEvidence(client?: SupabaseClient | null): Promise<BatchNext2P2Evidence> {
  const active = activeClient(client);
  const batchId = BATCH_NEXT2_P2_TARGET.batchId;
  const [
    batches,
    scopeItems,
    reconciliations,

...
import {
  ...
  type BatchNextP2EvidenceRow,
} from '@/features/productIdentity/batchNextP2ResumeSubmitContract';
...
function evidenceRows(data: unknown, label: string): BatchNextP2EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next P2 ${label} read returned an invalid row set.`);
  }
  return data as BatchNextP2EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next P2 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNextP2ResumeEvidence(client?: SupabaseClient | null): Promise<BatchNextP2Evidence> {
  const active = activeClient(client);
  const batchId = BATCH_NEXT_P2_TARGET.batchId;
  const [
    batches,
    scopeItems,
    reconciliations,

...
import {
  ...
  type BatchNext5P3EvidenceRow,
} from '@/features/productIdentity/batchNext5P3ResumePublishContract';
...
function evidenceRows(data: unknown, label: string): BatchNext5P3EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next 5 P3 ${label} read returned an invalid row set.`);
  }
  return data as BatchNext5P3EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next 5 P3 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNext5P3ResumeEvidence(
  phase: 'PRE' | 'POST',
  client?: SupabaseClient | null,
): Promise<BatchNext5P3Evidence> {
  const active = activeClient(client);
  const target = BATCH_NEXT5_P3_TARGET;

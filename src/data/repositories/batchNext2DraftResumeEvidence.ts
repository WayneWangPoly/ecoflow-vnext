...
export type BatchNext2ResumeEvidence = {
...
async function readRows(label: string, query: PromiseLike<{ data: unknown; error: unknown }>) {
  ...
  if (result.error) throw new Error(`Batch Next 2 ${label} read failed: ${errorMessage(result.error)}`);
  if (!Array.isArray(result.data)) throw new Error(`Batch Next 2 ${label} returned invalid data.`);
  return result.data as Array<Record<string, unknown>>;
}

export async function readBatchNext2ResumeEvidence(
  batchId: string,
  client?: SupabaseClient | null,
): Promise<BatchNext2ResumeEvidence> {

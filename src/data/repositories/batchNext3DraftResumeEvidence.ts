...
export type BatchNext3ResumeEvidence = {
...
async function readRows(label: string, query: PromiseLike<{ data: unknown; error: unknown }>) {
  ...
  if (result.error) throw new Error(`Batch Next 3 ${label} read failed: ${errorMessage(result.error)}`);
  if (!Array.isArray(result.data)) throw new Error(`Batch Next 3 ${label} returned invalid data.`);
  return result.data as Array<Record<string, unknown>>;
}

export async function readBatchNext3ResumeEvidence(
  batchId: string,
  client?: SupabaseClient | null,
): Promise<BatchNext3ResumeEvidence> {

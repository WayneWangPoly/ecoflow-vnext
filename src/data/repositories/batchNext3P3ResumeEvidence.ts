...
import {
  ...
  type BatchNext3P3EvidenceRow,
} from '@/features/productIdentity/batchNext3P3ResumePublishContract';
...
function evidenceRows(data: unknown, label: string): BatchNext3P3EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next 3 P3 ${label} read returned an invalid row set.`);
  }
  return data as BatchNext3P3EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next 3 P3 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNext3P3ResumeEvidence(
  phase: 'PRE' | 'POST',
  client?: SupabaseClient | null,
): Promise<BatchNext3P3Evidence> {
  const active = activeClient(client);
  const target = BATCH_NEXT3_P3_TARGET;

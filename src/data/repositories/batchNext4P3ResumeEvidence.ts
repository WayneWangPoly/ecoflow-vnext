...
import {
  ...
  type BatchNext4P3EvidenceRow,
} from '@/features/productIdentity/batchNext4P3ResumePublishContract';
...
function evidenceRows(data: unknown, label: string): BatchNext4P3EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next 4 P3 ${label} read returned an invalid row set.`);
  }
  return data as BatchNext4P3EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next 4 P3 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNext4P3ResumeEvidence(
  phase: 'PRE' | 'POST',
  client?: SupabaseClient | null,
): Promise<BatchNext4P3Evidence> {
  const active = activeClient(client);
  const target = BATCH_NEXT4_P3_TARGET;
  const batchId = target.batchId;
  const skuCodes = Object.keys(target.identities);
  ...
  const [
    ...
    readRows(`${label} family`, active
      ...
      .eq('created_in_batch_id', batchId).limit(11)),
    ...
    readRows(`${label} canonical inventory balance sentinel`, active
      ...
      .select('id,sku_id').in('sku_id', commercialSkuIds).limit(1)),

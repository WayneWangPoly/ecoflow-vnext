...
import {
  ...
  type BatchNext4P2EvidenceRow,
} from '@/features/productIdentity/batchNext4P2ResumeSubmitContract';
...
function evidenceRows(data: unknown, label: string): BatchNext4P2EvidenceRow[] {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== 'object' || Array.isArray(row))) {
    throw new Error(`Batch Next 4 P2 ${label} read returned an invalid row set.`);
  }
  return data as BatchNext4P2EvidenceRow[];
}

async function readRows(label: string, query: PromiseLike<ReadResult>) {
  const result = await query;
  if (result.error) throw new Error(`Batch Next 4 P2 ${label} read failed: ${errorMessage(result.error)}`);
  return evidenceRows(result.data, label);
}

export async function readBatchNext4P2ResumeEvidence(client?: SupabaseClient | null): Promise<BatchNext4P2Evidence> {
  const active = activeClient(client);
  const batchId = BATCH_NEXT4_P2_TARGET.batchId;
  const [
    batches,
    scopeItems,
    reconciliations,

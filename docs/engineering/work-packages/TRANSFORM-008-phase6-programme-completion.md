...
A final product-route audit found a separate completion defect: `PersonalisationProductivityPanel` remained implemented and governed, but the real Analytics desktop route rendered only `AnalyticsHealthConsole`. Therefore Saved Views, Quick Actions, governed Comparison Tray and Authoritative Export were not product-reachable from the current Analytics workspace even though their isolated contracts were green.
...
1. Mount the existing governed `PersonalisationProductivityPanel` exactly once inside the real Analytics workspace.
2. Preserve the existing Analytics route and role boundary; do not add a new route or privilege.
3. Preserve all TRANSFORM-008A/B/C authority contracts without widening browser authority.
4. Add a permanent static closure audit that fails if the Phase 6 surface becomes unreachable or regains browser-authored authority.
...
- `src/app/App.tsx` continues to route the Analytics desktop tab to `AnalyticsHealthConsole`.
- `AnalyticsHealthConsole` renders `PersonalisationProductivityPanel` exactly once.
...
4. `scripts/audit-transform-008-phase6-closure.mjs`.
...
8. Diff review confirming no migration, source write, guard bypass, privilege widening or unrelated feature work.

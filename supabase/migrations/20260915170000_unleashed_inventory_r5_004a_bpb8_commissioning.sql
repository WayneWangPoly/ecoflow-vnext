...
-- Engineering carrier only. This migration creates provenance/evidence controls and a
-- materialization bridge into the existing stocktake engine. It never approves a
-- stocktake and therefore never creates inventory quantity authority by itself.

create table if not exists public.ecoflow_unleashed_inventory_commissioning_sets (
...
create table if not exists public.ecoflow_unleashed_inventory_commissioning_locations (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.ecoflow_unleashed_inventory_commissioning_commands (
  id uuid primary key default gen_random_uuid(),
...
revoke all on public.ecoflow_unleashed_inventory_commissioning_sets from anon, authenticated;
revoke all on public.ecoflow_unleashed_inventory_commissioning_locations from anon, authenticated;
revoke all on public.ecoflow_unleashed_inventory_commissioning_commands from anon, authenticated;
...
$$;
revoke all on function public.ecoflow_r5_004a_payload_sha256(jsonb) from public, anon, authenticated;
...
begin
  ...
  select jsonb_build_object(
    ...
    'packageId','46c4c547-21d5-45df-b496-9506c26c36b8',
    'packageLevel','CARTON',
    'unitsPerPackage',1,
    ...
    'referenceAllocatedQtyTotal',coalesce(v_set.reference_allocated_qty_total,(
      select coalesce(sum(l.reference_allocated_qty),0) from public.ecoflow_unleashed_inventory_commissioning_locations l where l.commissioning_id=v_set.id
    )),
    'countedQtyTotal',coalesce(v_set.counted_qty_total,(
      select coalesce(sum(l.counted_qty),0) from public.ecoflow_unleashed_inventory_commissioning_locations l where l.commissioning_id=v_set.id
    )),
...
grant execute on function public.ecoflow_read_bpb8_inventory_commissioning_gate() to authenticated;
revoke execute on function public.ecoflow_read_bpb8_inventory_commissioning_gate() from anon, service_role;
...
begin
  ...
  select * into v_existing from public.ecoflow_unleashed_inventory_commissioning_commands where command_id=p_command_id;
  if found then return v_existing.result; end if;

  perform pg_advisory_xact_lock(hashtextextended('r5-004a-bpb8-start',0));

  select * into v_batch from public.ecoflow_unleashed_inventory_reference_batches
  where id='4cdb85d3-06d8-44bf-96bb-93660e10c3c9' for update;
  if not found
  ...
  select * into v_ref from public.v_ecoflow_unleashed_inventory_reference_rows
  where reference_row_id='7156d372-b12f-419e-b05e-2f12571ca525';
  if not found
  ...
  select * into v_phys from public.ecoflow_physical_skus where id='7dcaa2ed-a7db-4722-b91a-e8f17ffe2281';
  if not found or v_phys.identity_status <> 'ACTIVE' or v_phys.commercial_sku_id <> v_ref.commercial_sku_id
     or v_phys.preferred_package_id <> '46c4c547-21d5-45df-b496-9506c26c36b8'
  then raise exception 'R5_004A_PHYSICAL_IDENTITY_MISMATCH'; end if;

  select * into v_pkg from public.ecoflow_physical_sku_packages where id='46c4c547-21d5-45df-b496-9506c26c36b8';
  if not found or v_pkg.physical_sku_id<>v_phys.id or v_pkg.identity_status<>'ACTIVE'
     or upper(v_pkg.package_level)<>'CARTON' or v_pkg.units_in_base_unit<>1
  then raise exception 'R5_004A_PACKAGE_IDENTITY_MISMATCH'; end if;

  select * into v_barcode from public.ecoflow_physical_barcode_bindings
  where physical_sku_id=v_phys.id and barcode='19348045005009' and identity_status='ACTIVE';
  if not found then raise exception 'R5_004A_OPERATIONAL_BARCODE_MISMATCH'; end if;

  select * into v_set from public.ecoflow_unleashed_inventory_commissioning_sets
  where reference_row_id='7156d372-b12f-419e-b05e-2f12571ca525';
  ...
  insert into public.ecoflow_unleashed_inventory_commissioning_sets(
    ...
    '46c4c547-21d5-45df-b496-9506c26c36b8','CARTON','19348045005009',1,auth.uid()
  ) returning * into v_set;

  v_payload:=jsonb_build_object('referenceBatchId',v_batch.id,'referenceRowId',v_set.reference_row_id,'sku','BPB8');
  v_result:=jsonb_build_object('commissioningId',v_set.id,'status',v_set.status,'revision',v_set.revision,
    'sourceQtyOnHand',v_set.source_qty_on_hand,'inventoryAuthorityCreated',false);
  insert into public.ecoflow_unleashed_inventory_commissioning_commands(command_id,commissioning_id,command_type,actor_user_id,command_payload_sha256,result)
  values(p_command_id,v_set.id,'START',auth.uid(),public.ecoflow_r5_004a_payload_sha256(v_payload),v_result);
  return v_result;
end;
$$;

grant execute on function public.ecoflow_start_bpb8_inventory_commissioning(uuid) to authenticated;
revoke execute on function public.ecoflow_start_bpb8_inventory_commissioning(uuid) from anon, service_role;
...
begin
  ...
  select * into v_set from public.ecoflow_unleashed_inventory_commissioning_sets where id=p_commissioning_id for update;
  if not found or v_set.reference_row_id<>'7156d372-b12f-419e-b05e-2f12571ca525' or v_set.status<>'DRAFT'
    then raise exception 'R5_004A_DRAFT_BPB8_COMMISSIONING_REQUIRED'; end if;

  select * into v_location from public.ecoflow_warehouse_locations
  where upper(location_code)=upper(btrim(p_location_code)) and status='ACTIVE' limit 1;
  if not found then raise exception 'R5_004A_ACTIVE_PHYSICAL_LOCATION_REQUIRED'; end if;

  if exists(select 1 from public.ecoflow_unleashed_inventory_commissioning_locations where commissioning_id=v_set.id and location_id=v_location.id)
  ...
  ) values(v_set.id,v_location.id,v_location.location_code,p_reference_allocated_qty,p_counted_qty,left(btrim(p_note),2000),auth.uid());

  update public.ecoflow_unleashed_inventory_commissioning_sets set revision=revision+1,updated_at=clock_timestamp() where id=v_set.id returning * into v_set;

  v_payload:=jsonb_build_object('commissioningId',v_set.id,'locationCode',v_location.location_code,
    'referenceAllocatedQty',p_reference_allocated_qty,'countedQty',p_counted_qty,'note',left(btrim(p_note),2000));
  v_result:=jsonb_build_object('commissioningId',v_set.id,'status',v_set.status,'revision',v_set.revision,
    'locationCode',v_location.location_code,'referenceAllocatedQty',p_reference_allocated_qty,'countedQty',p_counted_qty,
    'inventoryAuthorityCreated',false);
  insert into public.ecoflow_unleashed_inventory_commissioning_commands(command_id,commissioning_id,command_type,actor_user_id,command_payload_sha256,result)
  values(p_command_id,v_set.id,'RECORD_LOCATION',auth.uid(),public.ecoflow_r5_004a_payload_sha256(v_payload),v_result);
  return v_result;
end;
$$;

grant execute on function public.ecoflow_record_bpb8_inventory_commissioning_location(uuid,text,numeric,numeric,text,uuid) to authenticated;
revoke execute on function public.ecoflow_record_bpb8_inventory_commissioning_location(uuid,text,numeric,numeric,text,uuid) from anon, service_role;
...
begin
  ...
  from public.ecoflow_unleashed_inventory_commissioning_locations where commissioning_id=v_set.id;
  ...
  set status='FINALIZED',reference_allocated_qty_total=v_alloc,counted_qty_total=v_count,
      accepted_count_variance=(v_count<>source_qty_on_hand),variance_reason=case when v_count<>source_qty_on_hand then left(btrim(p_variance_reason),2000) else null end,
      finalized_by=auth.uid(),finalized_at=clock_timestamp(),revision=revision+1,updated_at=clock_timestamp()
  where id=v_set.id returning * into v_set;

  v_payload:=jsonb_build_object('commissioningId',v_set.id,'referenceAllocatedQtyTotal',v_alloc,'countedQtyTotal',v_count,
    'acceptCountVariance',coalesce(p_accept_count_variance,false),'varianceReason',coalesce(p_variance_reason,''));
  v_result:=jsonb_build_object('commissioningId',v_set.id,'status',v_set.status,'revision',v_set.revision,
    'referenceAllocatedQtyTotal',v_alloc,'countedQtyTotal',v_count,'countVariance',v_count-v_set.source_qty_on_hand,
    'inventoryAuthorityCreated',false);
  insert into public.ecoflow_unleashed_inventory_commissioning_commands(command_id,commissioning_id,command_type,actor_user_id,command_payload_sha256,result)
  values(p_command_id,v_set.id,'FINALIZE',auth.uid(),public.ecoflow_r5_004a_payload_sha256(v_payload),v_result);
  return v_result;
end;
$$;

grant execute on function public.ecoflow_finalize_bpb8_inventory_commissioning(uuid,boolean,text,uuid) to authenticated;
revoke execute on function public.ecoflow_finalize_bpb8_inventory_commissioning(uuid,boolean,text,uuid) from anon, service_role;
...
begin
  ...
  select * into v_set from public.ecoflow_unleashed_inventory_commissioning_sets where id=p_commissioning_id for update;
  if not found or v_set.reference_row_id<>'7156d372-b12f-419e-b05e-2f12571ca525' or v_set.status<>'FINALIZED'
  ...
  select session_id,session_status,revision,created_at into v_session_id,v_session_status,v_session_revision,v_created_at
  from public.ecoflow_start_stocktake_session(
    'INITIAL','R5-004 BPB8 opening-balance canary',null,auth.uid(),false,
    'R5-004 BPB8 materialized from immutable ADL1 reference batch '||v_set.reference_batch_id::text,
    v_internal_command
  );

  for v_location in
    select * from public.ecoflow_unleashed_inventory_commissioning_locations where commissioning_id=v_set.id order by recorded_at,location_code
  loop
    v_internal_command:=gen_random_uuid();
    ...
    from public.ecoflow_record_stocktake_observation(
      v_session_id,v_location.location_code,'BPB8','8oz Kraft Soup Bowl 250ml','19348045005009','carton',1,
    ...
    if cardinality(v_exception_codes)>0 then raise exception 'R5_004A_UNEXPECTED_STOCKTAKE_EXCEPTION:%',array_to_string(v_exception_codes,','); end if;

    v_internal_command:=gen_random_uuid();
    select location_code,progress_status,revision into v_location.location_code,v_progress_status,v_progress_revision
    from public.ecoflow_complete_stocktake_location(v_session_id,v_location.location_code,'R5-004 BPB8 location evidence complete',v_internal_command);
  end loop;

  v_internal_command:=gen_random_uuid();
  select session_id,session_status,revision into v_session_id,v_session_status,v_session_revision
  from public.ecoflow_submit_stocktake_session(v_session_id,'R5-004 BPB8 canary ready for separate Owner/Admin approval gate',v_internal_command);
  if v_session_status<>'REVIEW' then raise exception 'R5_004A_MATERIALIZED_SESSION_NOT_REVIEW'; end if;

  update public.ecoflow_unleashed_inventory_commissioning_sets
  set status='MATERIALIZED',stocktake_session_id=v_session_id,materialized_by=auth.uid(),materialized_at=clock_timestamp(),revision=revision+1,updated_at=clock_timestamp()
  where id=v_set.id returning * into v_set;

  v_payload:=jsonb_build_object('commissioningId',v_set.id,'sourceRowSha256',v_set.source_row_sha256,'countedQtyTotal',v_set.counted_qty_total);
  v_result:=jsonb_build_object('commissioningId',v_set.id,'status',v_set.status,'revision',v_set.revision,
    'stocktakeSessionId',v_session_id,'stocktakeSessionStatus',v_session_status,'stocktakeSessionRevision',v_session_revision,
    'countedQtyTotal',v_set.counted_qty_total,'approvalRequired',true,'inventoryAuthorityCreated',false);
  insert into public.ecoflow_unleashed_inventory_commissioning_commands(command_id,commissioning_id,command_type,actor_user_id,command_payload_sha256,result)
  values(p_command_id,v_set.id,'MATERIALIZE',auth.uid(),public.ecoflow_r5_004a_payload_sha256(v_payload),v_result);
  return v_result;
end;
$$;

grant execute on function public.ecoflow_materialize_bpb8_initial_stocktake(uuid,uuid) to authenticated;
revoke execute on function public.ecoflow_materialize_bpb8_initial_stocktake(uuid,uuid) from anon, service_role;

comment on table public.ecoflow_unleashed_inventory_commissioning_sets is
  'R5-004 commissioning provenance bridge. This evidence layer creates no inventory authority; authority remains in separately approved stocktake sessions.';

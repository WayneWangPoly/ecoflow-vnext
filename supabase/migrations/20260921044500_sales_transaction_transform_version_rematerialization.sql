-- ECOFLOW-345-METRIC-ENG-R2
-- Transformation-version-aware sales transaction fact rematerialisation.
--
-- Production evidence showed that an InvoiceStatus mapping repair did not
-- rematerialise existing same-source-hash facts because source_version_hash only
-- represented raw/linkage hashes. This compensating migration adds an explicit
-- transformation contract version to document and line source_version_hash.
--
-- IMPORTANT:
-- - does not invoke analytics.refresh_sales_transaction_facts(...)
-- - does not activate/deprecate metrics
-- - does not call external providers
-- - does not mutate existing fact rows at migration apply time
-- A later separately-authorized refresh will SCD-close legacy transform versions
-- and materialise current rows under this transform contract.

begin;

do $preflight$
begin
  if to_regprocedure('analytics.refresh_sales_transaction_facts(timestamptz)') is null
     or to_regclass('analytics.fact_sales_transaction_document') is null
     or to_regclass('analytics.fact_sales_transaction_line') is null
     or to_regclass('analytics.metric_definition') is null then
    raise exception 'SALES_TRANSACTION_TRANSFORM_VERSION_PREREQUISITES_MISSING';
  end if;

  -- Trusted schema-only shadow copies can have an empty registry. In populated
  -- environments, preserve the existing DRAFT-only metric boundary.
  if exists(select 1 from analytics.metric_definition)
     and (
       not exists(
         select 1 from analytics.metric_definition
         where metric_key='revenue' and metric_version=2 and status='DRAFT'
       )
       or not exists(
         select 1 from analytics.metric_definition
         where metric_key='sales_orders' and metric_version=1 and status='DRAFT'
       )
       or not exists(
         select 1 from analytics.metric_definition
         where metric_key='average_revenue_per_order' and metric_version=1 and status='DRAFT'
       )
       or exists(
         select 1 from analytics.metric_definition
    ...
    raise exception 'SALES_TRANSACTION_METRIC_DRAFT_BOUNDARY_CHANGED';
  end if;
end;
$preflight$;
  ...
  begin
    ...
    select
      ...
      encode(extensions.digest(jsonb_build_array(
        'sales_transaction_transform_v2_invoice_status'::text,
...
comment on function analytics.refresh_sales_transaction_facts(timestamptz) is
  'Service-only snapshot refresh. Transform contract sales_transaction_transform_v2_invoice_status is included in document and line source_version_hash so mapping changes rematerialise through SCD on the next separately-authorized refresh. Reads canonical raw resources and maps InvoiceStatus for sales invoices, Status for credit notes. Preserves raw accounting evidence; no provider calls.';

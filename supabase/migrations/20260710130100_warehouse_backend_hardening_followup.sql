-- Follow-up corrections for warehouse backend hardening.
-- Preserve the barcode existence decision across later SELECT statements and
-- ensure all operational barcode KPIs use active packaging codes only.

begin;

update public.ecoflow_sku_barcode_registry
set units_per_barcode = greatest(round(coalesce(units_per_barcode, 1)), 1)
where units_per_barcode is null
   or units_per_barcode <= 0
   or units_per_barcode <> trunc(units_per_barcode);

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'ecoflow_barcode_units_positive_integer'
      and conrelid = 'public.ecoflow_sku_barcode_registry'::regclass
  ) then
    alter table public.ecoflow_sku_barcode_registry
      add constraint ecoflow_barcode_units_positive_integer
...
-- Rebuild operational barcode coverage from active codes only.
...
select
  coalesce((select count(*) from public.ecoflow_sku_barcode_registry where is_active), 0)::numeric as registered_barcodes,
  coalesce((select count(distinct sku) from public.ecoflow_sku_barcode_registry where is_active), 0)::numeric as covered_skus,

-- The cloud sync writes ordermentum_raw_orders only, while every operational
-- view (inbox, order lines, lifecycle, owner reports) reads business fields
-- from om_orders / om_order_items / om_invoices. Nothing has populated those
-- tables since the legacy importer was archived (om_orders stopped at
-- OMO2435, 2026-06-27), so newer orders never reached the UI. This migration
-- adds the missing projection stage and backfills the gap from raw history.

begin;

create or replace function public.ecoflow_om_safe_uuid(p_value text)
returns uuid
language sql
immutable
as $$
...
create or replace function public.ecoflow_om_safe_numeric(p_value text)
returns numeric
language sql
immutable
as $$
...
create or replace function public.ecoflow_om_safe_ts(p_value text)
returns timestamptz
language plpgsql
stable
as $$
...
create or replace function public.ecoflow_om_safe_bool(p_value text)
returns boolean
language plpgsql
immutable
as $$
    ...
    where public.ecoflow_om_safe_uuid(r.raw_payload->>'id') is not null
      and not exists (
        ...
        where o.id = public.ecoflow_om_safe_uuid(r.raw_payload->>'id')
          ...
          and (
            jsonb_typeof(r.raw_payload->'lineItems') <> 'array'
            or jsonb_array_length(r.raw_payload->'lineItems') = 0
...
-- Backfill the projection gap (no-op on an empty shadow database). Repeated
-- calls are idempotent: a projected order is skipped once om_orders is as
-- fresh as its raw payload.

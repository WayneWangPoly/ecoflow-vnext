-- ECOFLOW-345-FACT-MAT-R1-ENG: align the governed refresh with the
-- canonical raw snapshot resource namespace observed in production.
--
-- This is a compensating migration. The deployed FACT-ENG-R1 foundation
-- remains immutable. Fact-layer source_resource semantics remain frozen as
-- SalesInvoices / CreditNotes; only the raw snapshot lookup namespace changes.
-- No refresh is invoked by this migration.
...
begin
  if to_regprocedure('analytics.refresh_sales_transaction_facts(timestamptz)') is null then
    raise exception 'SALES_TRANSACTION_REFRESH_FUNCTION_MISSING';
  end if;
  if to_regclass('public.unleashed_raw_snapshots') is null then
    raise exception 'UNLEASHED_RAW_SNAPSHOTS_MISSING';
  end if;
end;
$preflight$;
...
comment on function analytics.refresh_sales_transaction_facts(timestamptz) is
  'Service-only snapshot refresh. Reads canonical raw resources sales_orders_history, sales_invoices and credit_notes while preserving frozen fact-layer source_resource labels SalesInvoices/CreditNotes. No provider calls.';

...
-- Fix: correct sku_lines CTE aggregation by removing max() from a non-grouped CTE.
-- Retains V4 self-reference fix and V3 line_index compatibility fix.
...
-- Reset V2 reporting views because Supabase may have partial/old definitions from earlier attempts.
drop view if exists public.v_ecoflow_top_skus_for_barcode_confirmation_v2 cascade;
drop view if exists public.v_ecoflow_ordermentum_sku_library_dashboard_v2 cascade;
drop view if exists public.v_ecoflow_sku_abc_analysis_v2 cascade;
drop view if exists public.v_ecoflow_sku_activity_summary_v2 cascade;
drop view if exists public.v_ecoflow_ordermentum_order_monthly_summary_v2 cascade;
drop view if exists public.v_ecoflow_ordermentum_all_orders_audit_v2 cascade;
drop view if exists public.v_ecoflow_ordermentum_order_lines_v2 cascade;
drop view if exists public.v_ecoflow_ordermentum_raw_order_line_extract_v2 cascade;
...
select
  ...
  l.invoice_number,
  l.source_line_id,
  row_number() over (partition by l.source_order_id order by l.source_line_id, l.external_sku_code, l.external_product_name)::int as line_index,
  l.external_sku_code,
...
where l.source = 'ordermentum_raw_invoices'
  -- Do not self-reference v_ecoflow_ordermentum_order_lines_v2 while creating it.
  -- Exclude invoice-fallback lines when the same order already has om_order_items lines.
  and not exists (
    select 1
    from public.om_orders o
    join public.om_order_items oi
      on oi.order_id = o.id
    where o.id::text = l.source_order_id
      and coalesce(oi.sku, '') = coalesce(l.external_sku_code, '')
  )
  -- Exclude invoice-fallback lines when raw synced order detail already exposed the same SKU.
  and not exists (
    select 1
    from public.v_ecoflow_ordermentum_raw_order_line_extract_v2 existing
    where existing.external_order_id = l.source_order_id
      and coalesce(existing.external_sku_code, '') = coalesce(l.external_sku_code, '')
  );
...
with listed as (
  ...
  select
    count(distinct b.sku_id) filter (where b.status = 'CONFIRMED')::bigint as barcode_confirmed_sku_count,
    count(distinct b.sku_id) filter (where b.status = 'CONFIRMED' and pl.level_code = 'CARTON')::bigint as carton_barcode_confirmed_sku_count,
    count(distinct b.sku_id) filter (where b.status = 'CONFIRMED' and pl.level_code = 'SLEEVE')::bigint as sleeve_barcode_confirmed_sku_count

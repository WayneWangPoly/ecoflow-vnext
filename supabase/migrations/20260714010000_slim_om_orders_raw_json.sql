-- The projection stored the full order-detail payload in om_orders.raw_json
-- (~26KB per row), so the homepage select=* against the active orders view
-- shipped tens of megabytes and died on the statement timeout even after the
-- semi-join was fixed. Keep only what downstream consumers read: scalars,
-- the address object, and a slim purchaser (store-site seeding needs id,
-- name, retailerPhone, deliveryInstructions, priceGroupId). Trim existing
-- rows the same way. The complete payload remains in ordermentum_raw_orders.
...
begin
  ...
  loop
    begin
      ...
      v_slim_raw := v_payload
        - 'lineItems' - 'invoice' - 'retailer' - 'supplier' - 'purchaser'
        - 'paymentOptions' - 'purchaserPaymentMethodTypes' - 'discounts'
        - 'boxes' - 'tags' - 'display' - 'issue' - 'chargedByUser' - 'placedBy';
      if jsonb_typeof(v_payload->'purchaser') = 'object' then
        v_slim_raw := v_slim_raw || jsonb_build_object('purchaser', jsonb_build_object(
          'id', v_payload->'purchaser'->'id',
          'name', v_payload->'purchaser'->'name',
          'retailerPhone', v_payload->'purchaser'->'retailerPhone',
          'deliveryInstructions', v_payload->'purchaser'->'deliveryInstructions',
          'priceGroupId', v_payload->'purchaser'->'priceGroupId'
        ));
      end if;
...
-- One-off trim of the rows the earlier projection already wrote fat.
update public.om_orders
set raw_json = (
  raw_json
    - 'lineItems' - 'invoice' - 'retailer' - 'supplier' - 'purchaser'
    - 'paymentOptions' - 'purchaserPaymentMethodTypes' - 'discounts'
  ...
  when jsonb_typeof(raw_json->'purchaser') = 'object' then jsonb_build_object('purchaser', jsonb_build_object(
    'id', raw_json->'purchaser'->'id',
    'name', raw_json->'purchaser'->'name',
    'retailerPhone', raw_json->'purchaser'->'retailerPhone',
    'deliveryInstructions', raw_json->'purchaser'->'deliveryInstructions',
    'priceGroupId', raw_json->'purchaser'->'priceGroupId'
  ))
  else '{}'::jsonb
end
where raw_json is not null;

analyze public.om_orders;

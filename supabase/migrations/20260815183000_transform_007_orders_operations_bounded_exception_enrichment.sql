-- Production repair for Orders desk statement_timeout (SQLSTATE 57014).
--
-- The list RPC previously counted current exceptions once for every active order
-- before filtering, sorting and LIMIT/OFFSET. That output-only enrichment could
-- multiply a scan of the exception snapshot by the full active-order set. Keep
-- the existing 8s authority boundary and all paging/filter/sort semantics, but
-- defer the exception count until after page_rows (page size is bounded to 100).
--
-- Do not increase statement_timeout here. The repair removes avoidable work.
  ...
  with freshness as (
    ...
    select
      ...
      jsonb_build_object(
        ...
        'active_exception_count', case when p.exception_snapshot_fresh then (
          select count(*)::integer
          from public.ecoflow_current_exception_snapshot x
          where (nullif(btrim(p.raw_order_id::text), '') is not null and x.raw_order_id = nullif(btrim(p.raw_order_id::text), ''))
             or (nullif(btrim(p.external_order_id::text), '') is not null and x.external_order_id = nullif(btrim(p.external_order_id::text), ''))
             or (nullif(btrim(p.external_order_number::text), '') is not null and x.external_order_number = nullif(btrim(p.external_order_number::text), ''))
             or (nullif(btrim(p.order_number::text), '') is not null and x.order_number = nullif(btrim(p.order_number::text), ''))
             or (nullif(btrim(p.invoice_number::text), '') is not null and x.invoice_number = nullif(btrim(p.invoice_number::text), ''))
